...
Development workshop for the forgent (fgOS) product. Product code, tests, and
specs live in `./repo` — an independent git repository nested here; see
`./repo/README.md` for the product itself.

## Workshop rules

- **A2 — phiên dev mở tại GỐC XƯỞNG.** Every dev session — human or agent —
  opens at this workshop root, not inside `./repo`. The workflow's hooks and
  state walk up from cwd, so starting inside `./repo` would put a session in
  the wrong project entirely.
- **`./repo` is an independent git repository.** It has its own `.git`, its
  own commit history, and its own remote (when added). This workshop has a
  separate, lighter git of its own (see D4); `./repo` is nested and untracked
  in it. The two trees are never one — commands, hooks, and `git status`
  never cross the boundary implicitly.
- **Where documentation lives (quick map — full table and rationale:
  `docs/history/repo-divorce/CONTEXT.md` decision D2):**

  | Lives in `./repo` (product) | Stays here (workshop) |
  |---|---|
  | `docs/specs/` (+ product reading-map) | `docs/history/` |
  | `docs/platform-foundations.md` | `plans/` |
  | `docs/backlog.md` | `docs/distillery/` |
  | `docs/routing-handoff-contract.md` | `docs/reference-learning-system.md`, `docs/naming.md` |
  | `docs/decisions/` | `upstreams/` |
  | | `.bee/`, `.claude/`, `.agents/`, `.codex/` |
