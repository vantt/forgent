...
function owner(l, root) {
  ...
  const dir = path.join(fgosDir(root), 'assignments', l.assignmentId), assignment = json(path.join(dir, 'assignment.json')), ledger = admissions(dir);
  const admitted = !ledger.corrupt && ledger.records.some((record) => record.runId === l.run.runId);
  if (!assignment || assignment.assignmentId !== l.assignmentId || l.run.assignmentId !== l.assignmentId || !admitted) return { complete: false };
...
function one(l, root, now) {
  const terminal = result(l), runResult = terminal.value, o = owner(l, root), phase = l.run.phase ?? (terminal.present ? 'settled' : l.run.status ?? 'unknown');
...
export function inspectDispatchRuntime(root, options = {}, { now = () => new Date().toISOString() } = {}) {
  ...
  if (selector.kind === 'assignment') {
    ...
    const facts = admissions(dir), records = all.filter((l) => l.assignmentId === selector.id), malformed = records.filter((l) => l.malformed), runs = records.filter((l) => !l.malformed), byId = new Map(runs.map((l) => [l.run.runId, l])), latest = facts.records.at(-1), currentIds = latest ? facts.records.filter((r) => r.attempt === latest.attempt && r.runId !== latest.runId ? true : r === latest).map((r) => r.runId) : [], current = currentIds.map((id) => byId.get(id)).filter(Boolean), absent = currentIds.filter((id) => !byId.has(id)), inspected = current.length === 1 ? one(current[0], root, now) : null, incomplete = facts.corrupt || malformed.length || absent.length, status = incomplete ? 'partial' : current.length > 1 ? 'conflicting' : inspected?.inspectionStatus ?? 'resolved';
    return { inspectionStatus: status, subject: { kind: 'assignment', id: selector.id, locations: records.map(project) }, observations: [{ kind: 'assignment-history', source: 'admission-generation-ledger', level: incomplete ? 'partial' : 'correlated', value: { delivery: facts.records.length ? 'started' : 'not-started', currentRunIds: currentIds, runIds: facts.records.map((r) => r.runId), missingMaterializations: absent, malformedMaterializations: malformed.map(project) } }], runObservation: inspected?.runObservation ?? null, runResult: inspected?.runResult ?? null, ...(!incomplete && status !== 'conflicting' && inspected?.recoveryAuthority ? { recoveryAuthority: inspected.recoveryAuthority } : {}), reconciliation: { state: incomplete || status === 'conflicting' ? 'manual-required' : 'not-needed', reason: incomplete ? 'Admission ledger and Run materialization are incomplete or corrupt.' : status === 'conflicting' ? 'Multiple current Runs were derived from admission facts.' : 'No stale local guard was observed.' }, links: { assignmentIds: [selector.id], coordinationIds: [], runIds: facts.records.map((r) => r.runId) } };
  }
  ...
  const complete = evidenceComplete && !malformed, hint = complete && active.length === 1 && !conflict ? authority(active[0], root) : null, status = conflict ? 'conflicting' : complete ? 'resolved' : 'partial';
