...
//     `done` yet waits — genuinely new, `approve` has no such check today.
 ...
 *   - `ready` — ids of `proposed` items whose `deps` are all `done` and
 ...
 *     `done` — merge-ready in isolation, but blocked on merge ORDER.
...
export function mergeReadiness(view) {
  ...
  for (const item of proposed) {
    ...
    const depsClear = deps.every((dep) => work[dep]?.status === 'done');
    if (depsClear) {
