...
// genuinely different events. This module closes that gap: `checkContiguity`
// is a read-only report (wired into the `events-jsonl-contiguous` fgos
// doctor check, `src/setup/registrations.mjs`); `fixContiguity` dedupes
// exact-duplicate lines, stable-sorts the rest by `ts`, and renumbers `seq`
// 1..N contiguously.
//
// Lives under src/state/ (kernel layer, alongside events.mjs, which it
// depends on) because src/setup/registrations.mjs -- a shipped, packaged
// module -- imports it at runtime; scripts/events-jsonl-contiguity.mjs is
// the thin CLI wrapper around this file, not the other way around.

import fs from "node:fs";
import { withEventsLock } from "./events.mjs";
