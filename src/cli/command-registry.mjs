...
export const COMMAND_REGISTRY = [
  ...
  {
    name: 'coordination',
    invoke: 'fgos coordination <run|show|close|actions|launch-master-loop|chain|recover>',
    description: 'Public door onto the CoordinationSession runtime (Step 08 Phase 07). "run" reads a request JSON file (--file), opens an agent-led or declared-protocol session, dispatches every declared step synchronously in V1 (a fan-out step\'s own branches dispatch concurrently), and reports the result -- through the existing, hardened session-engine.mjs exports only, never a forked dispatch path; session closing requires an explicit close step or the explicit "close" CLI door. "close" is the explicit terminal door: validates prerequisites and quorum, and records the terminal session outcome. "actions" is the read-only projection of session status and legal actions (coordination-actions.v1). Trusted global policy (--executor/--model/--tier) applies to every dispatch unless a per-actor override is declared in the request file itself (per-actor policy is trusted request-file content, never portable protocol; --model has no engine channel for kind:"declared-protocol" requests and is refused up front rather than silently dropped). "show" is a read-only status read (manifest + quorum + phase + event count, plus -- Step 09 Phase 02 R5 -- authorizations issued, dispositions recorded with post-terminal/ref-ownership marking, and which declared driver-authorized operations are still awaiting authorization, all sourced from the session\'s own replayed event log) -- no mutation, no external effect. "launch-master-loop" (Step 09 Phase 02 MVP4) is a thin composer for the shipped standalone-master-coordination-loop fixture: given --plan/--objective/--writer-id it builds a declared-protocol request covering ONLY the fixture\'s required first pass (produce/review/red-team) and runs it through the same door as "run" -- it never authorizes or dispositions the fixture\'s driver-authorized revision/recheck operations itself. Its result always carries a `nextAction` message naming the coordination id and what to do next (R5), including a plain-terms explanation when the session could not close because a driver-authorized step is still pending. "chain" (Step 09 Phase 02 R2-R4) is a read-only status read across a WHOLE chain of cell-sessions: given a `<track>` prefix, it lists every session id under `.fgos/coordination/sessions/` starting with `<track>--`, renders each cell\'s status/phase/last disposition/pendingDriverAuthorizations/Assignment ids (reusing "show"\'s own read path per session, never a second derivation), names the most-recently-created still-`active` cell as `activeCell` (or `null`), and a plain-text `nextAction` hint for it -- reconstructed entirely from each session\'s own event log, never a new persisted plan/status object; no mutation, no external effect, no cache file written. "recover" is the CAS-guarded recovery door for a COORDINATION-SESSION-owned Run (schema-2 sessions only) -- the session-scoped analog of `dispatch recover`. Called with no --action, it returns a pure, ephemeral SessionRecoveryRecommendation (snapshot digest, expected event sequence, expected Run control epoch, a fresh action key, an expiry, and the recommended action of observe/collect/settle/close/park) derived from the SAME shared pure evaluators (`read-evaluators.mjs`) the write door itself is built on -- no write, no session event, safe to call any number of times. Called WITH --action (plus all five --expected-* CAS fields naming the exact recommendation being applied), it re-verifies CURRENT state under the session\'s own lock, refuses a changed snapshot/event-sequence/Run-control-epoch as "plan-stale", a missing/replaced current driver as "needs-input", a premature "close"/"collect"/"settle" (closing while an Assignment is still in flight stays a visible hazard, never silently converted into success) as "refuse", and a repeated call with an already-consumed --action-key as "already-applied" -- before recording exactly one `recovery-command-recorded` event carrying the action key. Never appends any OTHER session event and never invokes the session\'s own unconditional close-after-steps; `dispatch recover` remains the ONLY door for a STANDALONE Run and appends no session event at all -- the two doors never blur that boundary. `--cwd <path>` (R7) names the worker/session working directory `run`/`show`/`close`/`actions`/`launch-master-loop`/`chain`/`recover` operate against -- distinct from `--dir` (which names the main checkout\'s `.fgos/`, i.e. the repo root); omitted, behavior is byte-identical to today (the working directory defaults to the resolved repo root).',
    parameters: {
      type: 'object',
      properties: {
        sub: { type: 'string', description: 'Sub-verb (positional).', enum: ['run', 'show', 'close', 'actions', 'launch-master-loop', 'chain', 'recover'] },
        file: { type: 'string', description: '"run"/"close" only: path to the request JSON file.' },
        id: { type: 'string', description: '"show"/"actions"/"recover" only: the coordinationId to read/project/recover (positional or --id).' },
        ...
        objective: { type: 'string', description: '"launch-master-loop" only: the session objective (also folded into the produce step\'s own objective text).' },
        'writer-id': { type: 'string', description: '"launch-master-loop" only: trusted driver/operator identity opening the session.' },
        'coordination-id': { type: 'string', description: '"launch-master-loop" only: explicit session id; omitted lets the engine auto-generate one.' },
        'fixture-version': { type: 'string', description: '"launch-master-loop" only: if given, must match the registered fixture\'s metadata.version or the call is refused before composing a request.' },
        executor: { type: 'string', description: '"run"/"launch-master-loop": global trusted executor id, applied to every dispatch unless a per-actor "actors[].executor" override is declared in the request file.' },
        model: { type: 'string', description: '"run"/"launch-master-loop": global trusted model id -- kind:"agent-led" requests only; refused for kind:"declared-protocol" (no engine policy channel today).' },
        tier: { type: 'string', description: '"run"/"launch-master-loop": global trusted minimum tier, applied to every dispatch unless a per-actor "actors[].tier" override is declared in the request file.' },
        json: { type: 'boolean', description: '"show"/"actions"/"chain": accepted as a no-op -- the envelope is always JSON.' },
        cwd: { type: 'string', description: 'Worker/session working directory for "run"/"show"/"close"/"actions"/"launch-master-loop"/"chain"/"recover" (R7); distinct from --dir (the main checkout root). Omitted defaults to the resolved repo root, byte-identical to before this flag existed.' },
        action: { type: 'string', description: '"recover" only: one of observe|collect|settle|close|park, exactly as returned by a prior "recover" call\'s own `action` field. Presence of this flag is what selects the apply path over the observe path; giving it requires all five --expected-* fields and --action-key too.' },
        'expected-snapshot': { type: 'string', description: '"recover" apply only: the `snapshotDigest` from the recommendation being applied.' },
        'expected-event-seq': { type: 'string', description: '"recover" apply only: the `expectedEventSeq` from the recommendation being applied.' },
        'expected-run-control-epoch': { type: 'string', description: '"recover" apply only: the `expectedRunControlEpoch` from the recommendation being applied.' },
    ...
    examples: [
      ...
      'fgos coordination run --cwd ../worker-worktree --file request.json',
      ...
      'fgos coordination recover coord_abc123 --action observe --expected-snapshot <digest> --expected-event-seq 3 --expected-run-control-epoch 0 --expected-expires-at <iso> --action-key <key>',
    ],
    touchesState: true,
    requiresExistingStore: false,
    externalEffect: true,
