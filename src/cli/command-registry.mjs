  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        json: { type: 'boolean', description: '"show"/"chain": accepted as a no-op -- the envelope is always JSON.' },
        cwd: { type: 'string', description: 'Worker/session working directory for "run"/"show"/"launch-master-loop"/"chain"/"recover" (R7); distinct from --dir (the main checkout root). Omitted defaults to the resolved repo root, byte-identical to before this flag existed.' },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        action: { type: 'string', description: '"recover" only: JSON-encoded action object to apply, exactly as returned by a prior "recover" call\'s own `action` field. Presence of this flag is what selects the apply path over the observe path; giving it requires all four --expected-* fields and --action-key too. "reconcile plan" only (different meaning): the plain action-kind string to plan, one of "clear-cwd-lock" (default), "collect-result" (requires --run), or "clear-assignment-claim" (requires --assignment).' },
