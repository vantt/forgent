...
export const COMMAND_REGISTRY = [
  ...
  {
    name: 'coordination',
    invoke: 'fgos coordination <start|status|run|close|show|chain|recover|operation|authorize-and-dispatch|fan-out|contribution|human-turn|disposition|clean|inspect>',
    description: 'Public door onto the CoordinationSession runtime (Step 08 Phase 07, Step 09 Phase 02). Provides both semantic step commands ("start", "status", "operation", "authorize-and-dispatch", "fan-out", "contribution", "human-turn", "disposition", "close", "clean", "inspect") and raw request file execution ("run", "close --file"). "start" composes a validated start request and opens a declared-protocol or agent-led session. "status" and "inspect" provide read doors for coordination-actions.v1, providing compact status, readiness, blockers, and legal actions (with optional --detail and --replay). "operation", "authorize-and-dispatch", "fan-out", "contribution", "human-turn", "disposition", and "close" execute single semantic actions under atomic session locking, checking actionKey freshness and driver identity. "run" reads a request JSON file (--file) and dispatches declared steps synchronously in V1. "show" and "chain" provide read-only projections. "clean" safely removes ephemeral locks without mutating active sessions. "recover" provides CAS-guarded session recovery.',
    parameters: {
      type: 'object',
      properties: {
        sub: { type: 'string', description: 'Sub-verb (positional).', enum: ['start', 'status', 'run', 'close', 'show', 'chain', 'recover', 'operation', 'authorize-and-dispatch', 'fan-out', 'contribution', 'human-turn', 'disposition', 'clean', 'inspect'] },
        ...
        detail: { type: 'boolean', description: '"status"/"inspect" only: include snapshot digest and facts in output.' },
        replay: { type: 'boolean', description: '"status"/"inspect" only: include full replayed events in output.' },
        'dry-run': { type: 'boolean', description: '"clean" only: simulate cleanup without deleting files.' },
        force: { type: 'boolean', description: '"clean" only: force cleanup on active session artifacts.' },
    ...
    examples: [
      ...
      'fgos coordination clean coord_abc123',
      'fgos coordination inspect coord_abc123 --json',
