...
function readDecisionIds(projectRoot) {
  ...
  if (!canonicalPrefix.every((line, index) => lines[index] === line)) return new Set();
  const headerIndex = canonicalPrefix.length;
  if (lines[headerIndex] !== '| Scope | Decision | Rationale | Recorded |') return new Set();
  if (lines[headerIndex + 1] !== '|---|---|---|---|') return new Set();

  const ids = new Set();
  for (const line of lines.slice(headerIndex + 2)) {
    if (!line.trim()) continue;
    const cells = line.split('|');
    if (cells.length !== 6 || cells[0] !== '' || cells[5] !== '') return new Set();
    const match = /^\s*(D-ADR\d{4}):/.exec(cells[2]);
