...
function readDecisionIds(projectRoot) {
  ...
  if (
    !/^# Decisions index$/m.test(content) ||
    !content.includes('generated by `fgos decision-index`') ||
    !/^\|\s*Scope\s*\|\s*Decision\s*\|/m.test(content)
  ) {
    return new Set();
  }
  const ids = new Set();
  for (const line of content.split(/\r?\n/)) {
    const match = /^\|\s*[^|]+\|\s*(D-ADR\d{4})(?::|\s*\|)/.exec(line);
