...
const replacement = `export function openSession(
  ...
  try {
    ...
    if (schemaVersion === "3" && definitionRef && opts.resolvedDefinition) {
      ...
      const digest = crypto ? crypto.createHash('sha256').update(snapshotStr).digest('hex') : Buffer.from(snapshotStr).toString('base64');
...
fs.writeFileSync('src/runner/coordination/store.mjs', before + replacement + after);
