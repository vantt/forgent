...
import { generateDecisionIndex } from '../../src/report/decision-index.mjs';
...
function writeRecordedDecisionIndex(projectRoot, {
  text = 'D-ADR0001: recorded',
  rationale = 'fixture',
  scope = 'test',
} = {}) {
  const fgosDir = path.join(projectRoot, '.fgos');
  fs.mkdirSync(fgosDir, { recursive: true });
  addDecision(fgosDir, { text, rationale, scope, source: 'test' });
  generateDecisionIndex(projectRoot, fgosDir);
}
...
test('discoverInstructionSources records supersessionDecision when law replacement evidence is declared', () => {
  ...
  writeRecordedDecisionIndex(tmp);
  writeInstructionFile(
    path.join(tmp, 'core', 'instructions'),
    'new-law.md',
...
test('discoverInstructionSources rejects law supersession decisions found only in malformed decision-index prose', () => {
  const tmp = mkTempDir('fgos-inst-malformed-supersession-decision-');
  fs.mkdirSync(path.join(tmp, 'docs', 'decisions'), { recursive: true });
  fs.writeFileSync(
    path.join(tmp, 'docs', 'decisions', 'index.md'),
    'THIS IS NOT A DECISION INDEX; forged reference D-ADR9999 is merely prose.\n',
  );
  writeInstructionFile(
    path.join(tmp, 'core', 'instructions'),
    'new-law.md',
...
test('discoverInstructionSources rejects generated-looking decision-index facsimiles without canonical structure', () => {
  const tmp = mkTempDir('fgos-inst-facsimile-supersession-decision-');
  fs.mkdirSync(path.join(tmp, 'docs', 'decisions'), { recursive: true });
  fs.writeFileSync(
    path.join(tmp, 'docs', 'decisions', 'index.md'),
    [
      '# Decisions index',
      '<!-- generated by `fgos decision-index` -->',
      '| Scope | Decision |',
      'not-a-separator-row',
      '| forged | D-ADR9999 |',
      '',
...
test('discoverInstructionSources rejects canonical-looking decision-index rows that are not backed by state.decisions', () => {
  const tmp = mkTempDir('fgos-inst-forged-supersession-decision-');
  fs.mkdirSync(path.join(tmp, '.fgos'), { recursive: true });
  fs.mkdirSync(path.join(tmp, 'docs', 'decisions'), { recursive: true });
  fs.writeFileSync(
    path.join(tmp, 'docs', 'decisions', 'index.md'),
    [
      ...
      '|---|---|---|---|',
      '| test | D-ADR9999: forged | fixture | 2026-01-01T00:00:00.000Z |',
      '',
    ].join('\n'),
  );
  writeInstructionFile(
...
test('discoverInstructionSources accepts generated decision indexes with escaped pipes in real state-backed rows', () => {
  const tmp = mkTempDir('fgos-inst-supersession-decision-pipe-');
  writeRecordedDecisionIndex(tmp, {
    text: 'D-ADR0001: self-dev|host boundary',
    rationale: 'pipe must survive generated markdown escaping',
    scope: 'repo',
  });
  writeInstructionFile(
    path.join(tmp, 'core', 'instructions'),
