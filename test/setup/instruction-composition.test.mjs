...
function registryLaw(projectRoot, id, overrides = {}) {
  const meta = {
    id,
    kind: 'law',
    ...(overrides.supersedes ? { supersedes: overrides.supersedes } : {}),
    ...(overrides.supersessionDecision ? { supersessionDecision: overrides.supersessionDecision } : {}),
  };
  return compileInstructionUnit(meta, overrides.body ?? `${id} body`, '', {
    projectRoot,
    fullPath: path.join(projectRoot, 'core', 'instructions', `${id}.md`),
    sourcePath: `core/instructions/${id}.md`,
    rootType: 'platform',
    rootOwner: 'platform',
    knownOwners: new Set(['platform']),
    fgosDir: path.join(projectRoot, '.fgos'),
  });
}
...
test('composeInstructionSet rejects caller-forged law supersession verification flags', () => {
  const result = evaluateInstructionComposition([
    unit('old-law', { kind: 'law' }),
    unit('new-law', {
      kind: 'law',
...
test('composeInstructionSet rejects caller-provided law supersession allowlists', () => {
  const result = evaluateInstructionComposition([
    unit('old-law', { kind: 'law' }),
    unit('new-law', {
      kind: 'law',
  ...
  ], { validSupersessionDecisions: ['D-ADR0001'] });
...
test('composeInstructionSet accepts law supersession only with verified decision evidence', () => {
  const root = mkTempDir('fgos-composition-verified-law-');
  writeRecordedDecisionIndex(root);

  const set = composeInstructionSet([
    registryLaw(root, 'old-law'),
    registryLaw(root, 'new-law', {
      supersedes: ['old-law'],
      supersessionDecision: 'D-ADR0001',
    }),
  ]);
...
test('composeInstructionSet accepts law supersession when a real generated decision index escapes pipes', () => {
  const root = mkTempDir('fgos-composition-verified-law-pipe-');
  writeRecordedDecisionIndex(root, {
    text: 'D-ADR0001: self-dev|host boundary',
    rationale: 'generated rows escape pipes',
    scope: 'repo',
  });
