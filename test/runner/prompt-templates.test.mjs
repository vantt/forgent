...
test('renderTemplate(worker-prompt-skill-pointer.txt, ...) golden output — no-feedback shape, byte-for-byte', () => {
  const rendered = renderTemplate('worker-prompt-skill-pointer.txt', {
    ...
    verify: 'npm test',
    domain: 'coding',
    skillPath: '.claude/skills/fgos-workflow/fgos-executing/SKILL.md',
  });
...
anything else, read .claude/skills/fgos-workflow/fgos-executing/SKILL.md in your own checkout — it is the same skill
