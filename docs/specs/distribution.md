...
| 7 | Doctor check | One named diagnostic `fgos doctor` reports on | not a fixed list — an extensible registry (`src/setup/registrations.mjs`'s `registerCheck`); any module can add its own check independently of registering a config-default or a fix (tsk-2cs D2). Today's registered checks: `node-version-and-git`, `shell-integration-sourced`, `config-not-stale`, `main-checkout-hook-wired`, `tool-registry-configured`, `root-drift`, `config-awareness`, `dependencies-installed`, `gate-bypass-configured` — this list grows without a spec update whenever a module registers a new one | yes | — |
