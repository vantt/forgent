...
- Phép kiểm cây-sạch của `return` (RUL19) — loại trừ `.fgos/` qua `isFgosOnlyStatusLine` — chỉ khớp status line có tiền tố `.fgos/` LITERAL. Khi `cwd` (nơi `return` chạy) là một thư mục con của repo git thật (không phải root), `git status` báo path tương đối theo dạng `<subdir>/.fgos/...`, nên exclusion không bao giờ khớp: `return` từ chối "working tree not clean" dù chỉ có state store của chính item đó tự sinh, và cũng bị chặn bởi BẤT KỲ file KHÁC đang uncommitted ở bất kỳ đâu trong repo — phép kiểm scan toàn bộ `git status`, không giới hạn theo cwd/subdirectory đang chạy. Tìm ra thật qua dogfood một fixture sống trong thư mục con (`docs/history/fgos-sample-testbed/`, hai lần xác nhận độc lập, 2026-07-18); chưa sửa, xem `docs/backlog.md` STR60.
