...
- **Blocked when:** item không tồn tại — `validation`; item không ở
  ...
  trên một thư mục không-git vẫn hoạt động như trước. `approve --github`
  KHÔNG bị CẢ HAI chặn này (không đụng cây cục bộ — xem "Cổng duyệt qua
  GitHub"). Không nhánh chặn nào ghi sự kiện.
- **What changes:**
