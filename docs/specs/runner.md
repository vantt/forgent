...
4. **Attestation bền:** Mỗi dispatch sinh một bản ghi `confinement-attestation.v1` lưu kèm kết quả dispatch, minh bạch từng kênh bảo vệ (`hostWrite`, `hostRead`, `networkEgress`, `process`, `home`, `session`, `workspace`) và các write grant được cấp.
