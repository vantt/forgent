...
## Dispatch operability planned design (2026-09-14)

Track `plans/260914-dispatch-operability-evidence-attribution/` reached
`READY` as design authority only. It does not ship runtime behavior and does not
authorize implementation in that track. A future implementation track may build
the following planned capabilities:
...
reassign through reconciliation, no cross-session authority, no same-`taskKey`
semantic change, no host OOM/provider-limit prevention, and no direct-unit-only
proof for shipped capabilities.
