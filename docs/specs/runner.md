...
decisions: [feed7428, 14396a5c, 1a80b4d3, 9a19eea5, 96a65365, a7c099af, 43f257ae, 44936500, e1218b22, 6f2cbc47, a30a3d3c, 1359ab5e, cfae0120, 22699c61, 04a6cd05, 396d9d9e, 2e92b7a5, f0c40acc, 5a6900b2, 8575f1a3, c8df2479, cb09d6fd, b1aa1bdc, caecb9d1, 9b141173, a3176299, 140eb8a4, 76b7a36b, 8d04bba3, 1cd895e1, 38160a70, c11322cb, 2ac16176, f8a3a5d9, 3d4ea29c, 3c8e5926, 342102b9, d4c59ba2, 644916a4, ef6ed305, a4fe4c2b, f69951df, 5208dfe9, 8cf7effe, 7bbe6315]
coverage: full
---
...
- `fgos review <id>` / `fgos approve <id> [--timeout <ms>] [--acknowledge-iron-law]` / `fgos reject <id> --reason "..."` (ngoài vòng runner, gọi bởi người vận hành) — cổng duyệt PR nội bộ cho một đề xuất `proposed` đã sẵn, MỘT cổng cho cả nguồn runner lẫn pull-door; `approve` nguồn runner còn chạy phán Iron Law trước khi merge (self-improve loop P13 Slice 3, D16/D17) — xem "Cổng duyệt PR nội bộ" dưới
...
|---|-------|---------|--------|----------|---------|
| 1 | Cấu hình runner (file committed ở gốc repo) | Chính sách thực thi — LÀ CONFIG THỰC THI ĐƯỢC: ai sửa nó điều khiển tiến trình được spawn (đầu vào tin cậy) | `executor` — mẫu lệnh gọi trợ lý (thay thế {prompt}/{model} theo từng phần tử, không bao giờ qua shell), tùy chọn `adapter` (mặc định `cli-spawn`, xem C9 v2 dưới) · `executors` — TÙY CHỌN, bảng tier→executor riêng (cùng hình dạng `executor`); tier không khai riêng rơi về `executor` chung (P41/D a4fe4c2b) · `models` — bảng tier→model (light/standard/heavy) · `timeoutMs` — trần thời gian một worker | yes | có sẵn bản mặc định |
...
### Xem live output worker khi đang chạy (live tee, P39)
...
- **Blocked when:** item không tồn tại — `validation`; item không ở
  ...
  thông điệp nêu tên đúng cờ/module đã khớp (self-improve loop P13 Slice 3,
  ...
  không bao giờ gọi qua verb đó (P44 — sổ đăng ký ở trên chỉ bắt được
...
- **What changes:**
  ...
  - `approve <id>` — nguồn `runner`: TRƯỚC MỌI thao tác git, chạy phán Iron
    Law (self-improve loop P13 Slice 3, D16/D17 — xem "Iron Law" dưới) trên
...
Bước vào của vòng tự cải thiện (self-improve loop, P13 — CONTEXT.md
...
- **What changes:**
  - `fgos evolve` (không `--pick`/`--submit`) và `fgos evolve --pick <id>` —
    ...
    - `fgos evolve --pick <id>` — xếp hạng lại CÙNG một view rồi tìm `id`
      ...
      cần xử lý.
  - `fgos evolve --submit <id>` (self-improve loop P13 Slice 3, D15) — HÀNH
...
- **Afterwards:** `evolve`/`evolve --pick` — người vận hành thấy đúng
  ...
  **Tương tác đã biết (chứng minh bằng e2e thật, P13 Slice 3):** vì Iron
...
### Iron Law — phân loại rủi ro của một candidate fix (self-improve loop P13 Slice 2/3)
...
- **Runs when:** gọi từ bên trong `approve` nguồn `runner` (Cổng duyệt PR nội
  ...
  lần một đề xuất nguồn runner được duyệt (self-improve loop P13 Slice 3,
  ...
  **Ranh giới CHỦ Ý:** chỉ đề xuất nguồn `runner` mới đi qua phép thử này —
  ...
  trống bị bỏ sót (per D16, xem R37).
...
- **R32 (vòng tự cải thiện chỉ nhắm vào chính repo sản phẩm, không phải tính năng mở cho host ngoài).** Toàn bộ vòng self-improve (Gate A, Iron Law, và Gate B wiring — cả ba đã xây, P13 hoàn tất) tác động lên chính `repo/src` của fgOS — công cụ fgOS tự soi lại chính nó, không phải một khả năng fgOS cấp cho project khác mà nó đang điều phối (per D1 self-improve-loop / c8df2479).
...
- Danh sách module của phép thử Iron Law (Data Dictionary #10, D10/D14) vẫn là danh sách MINH HỌA, không đóng khung — có thể còn module năng-lực-liên-quan khác (vd các module domain/kernel khác trong `src/state/`, `src/runner/`) chưa được liệt kê mà lẽ ra đủ tư cách theo phép thử năng lực gốc của D10; mở lại khi vận hành thật (wiring đã live, R37) cho thấy một trường hợp bỏ sót thật.
- Merge qua GitHub (`approve --github`) không dọn nhánh sau khi xong — nhánh cục bộ `fgw/<id>` VÀ bản đã đẩy lên remote gốc đều còn nguyên, khác với merge cục bộ (tự dọn cả hai, xem R21) — không có cơ chế dọn nhánh đã merge phía-server hôm nay; chấp nhận biết-nhưng-chưa-sửa cho slice này (github-adapter S3).
...
- `src/runner/loop.mjs`'s `parseDiscoveredBlocks(output)` + `captureDiscoveredWork(...)` — kênh báo việc-phát-hiện (xem "Báo việc-phát-hiện từ trợ lý" trên, R45): tách mọi khối lồng `fgos-discovered` (JSON, chỉ cần `title` khác rỗng) khỏi output đã thu của trợ lý, fail-safe (thân JSON hỏng/thiếu `title`/không phải object đều bị bỏ qua, không throw); gọi đúng MỘT LẦN trong `finally` bọc vòng thử lại của `dispatchClaimedItem`, tại kết cục cuối (không gọi lại giữa các lần thử nội bộ); mỗi khối hợp lệ đi qua `write-queue.mjs`'s `enqueue` (cùng cửa ghi tuần tự D12) — bên trong đó: cắt còn tối đa `DISCOVERY_CAP` (=20) khối trước khi lặp (phần dư log rồi bỏ), rồi với mỗi khối: quét view hiện hành tìm item có `discoveredFrom` khớp item đang thi công VÀ tên khớp sau khi chuẩn hoá (trim + hạ chữ) — khớp thì log rồi bỏ qua (không tạo); không khớp mới gọi `generateId`/`classify`/`addWork` (đóng dấu `discoveredFrom: item.id`, `stage: 'clarify'`); `dispatch.mjs`'s văn bản prompt worker mô tả kênh khối lồng này cho trợ lý (nguyên tắc "chỉ báo, không tự ghi" — trợ lý vẫn không bao giờ được gọi `fgos`/đụng `.fgos/`)
