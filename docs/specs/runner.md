...
- **Afterwards:** item pass-through hoặc vừa sinh đủ con (gốc chuyển
  ...
  dang dở). **Claim-release (claim-lock §3b):** Khi một gốc chuyển sang
  `executing`, nếu một claim `pick` đang sống ở stage `decompose` được giữ
  (item ở `status: doing`), item được đặt lại về `status: todo` — giải phóng
  claim để phiên khác (hay cùng phiên) có thể gọi `pick <id>` lại ở stage
  `executing` với workspace cùng một branch (claim-release lifecycle cho
  clarify/decompose-phase workflow, per claim-lock §3a/§3c); item cần người quyết đậu ở `awaiting-human` mang đề xuất chia,
...
`.fgos/config.json`'s `runner.executor.args` (fallback: `.fgos-runner.json`'s `executor.args`) đã là một mẫu Host Adapter (Data
...
- Item vừa qua quét làm-rõ trong CÙNG lượt chạy: nếu deps cũng đã xong, vòng dispatch bên dưới nhặt được ngay, không cần đợi lượt chạy kế tiếp — chứng minh bằng e2e một lượt `--once` duy nhất đưa item từ `clarify` tới `awaiting-approval`.
- Ngã-ngũ tự động của runner trong một lượt dispatch thật (quét làm-rõ, nhận việc, đề xuất, đỗ) đều mang đúng `role` = `runner`; ngã-ngũ đóng tay qua CLI mang `role` = `human` — chứng minh bằng benchmark F4 chạy qua binary thật (không chỉ fixture), round 2: 6/6 tiêu chí đạt.
...
- Item đơn giản đi qua quét làm-rõ rồi quét chia-việc trong CÙNG một lượt chạy `--once`: cả hai stage (`clarify → decompose → executing`) hoàn tất và item tới `awaiting-approval` trước khi lượt chạy đó kết thúc — chứng minh bằng e2e một lượt duy nhất qua binary thật.
...
- `.fgos/config.json`'s `runner` section (tsk-5vf D1/D2, thay `.fgos-runner.json` flat cũ — file cũ vẫn đọc được làm fallback khi file mới chưa tồn tại, `fgos setup` là verb thực hiện move thật, không tự xoá file cũ) — config committed (executor template + models light/haiku, standard/sonnet, heavy/opus + timeoutMs); `executor.args` mang `--permission-mode acceptEdits` + `--allowedTools "Bash(git add:*),Bash(git commit:*)"` (quyền TỐI THIỂU, xem RUL6); khối `parallel` TÙY CHỌN — `maxRoots`/`maxLeavesPerRoot` (Data Dictionary #6, mặc định trong-code 4/4 khi khối vắng mặt, mọi config cũ vẫn chạy không cần sửa); `src/runner/dispatch.mjs`'s `ensureRunnerConfigForDir(dir)` — shared-file-aware, gọi `ensureRunnerConfig(legacyPath)` không đổi khi chỉ có file cũ (byte-for-byte parity): tại đường MẶC ĐỊNH (không `--config`) vắng cả hai file, gọi `detectAssistantCli()` (quét PATH thuần, không spawn — `KNOWN_ASSISTANT_CLI_NAMES`) rồi viết `DEFAULT_RUNNER_CONFIG` với executor từ `SUPPORTED_EXECUTOR_TEMPLATES[detected]` khi trợ lý dò được có mẫu đã kiểm chứng (hôm nay chỉ `claude`, y hệt `DEFAULT_RUNNER_CONFIG.executor`), hoặc executor placeholder tự-báo-lỗi khi không (RUL48b) — rồi mới nạp, kèm một dòng thông báo tên file + executor được chọn (RUL48); `bin/fgos.mjs`'s verb `discover` và `bin/fgos-runner.mjs`'s vòng chính đều gọi `ensureRunnerConfigForDir` cho nhánh đường mặc định, `loadRunnerConfig` thẳng cho nhánh `--config` tường minh; đọc qua `mergeWithGlobalConfig` trước khi tách phần `runner` (project luôn thắng `~/.fgos/config.json` theo key, tsk-5vf D2)
