...
- **Afterwards:** người vận hành thấy việc ở `proposed` + nhánh để review; việc phụ thuộc CHƯA mở (chờ duyệt/merge → `done`); vòng --once thứ hai không giao lại việc nào (frontier trống). Kết cục cuối của lượt được in ra dưới dạng phong bì máy-đọc (xem RUL61 dưới), dòng cuối cùng của output.
...
- **What changes:**
  ...
  - `approve <id>` — nguồn `runner`: TRƯỚC MỌI thao tác git, chạy phán Iron
    ...
    commit` (hoàn tất merge) rồi `proposed → done` mang **role `human`**
  ...
  - `reject <id> --reason` — `proposed → todo` mang `reason` + role human;
...
- **What changes:**
  ...
  - `approve <id> --github --pr <n>` — gọi merge của GitHub trên đúng PR số
    `n`. Sạch → `proposed → done` mang role `human` (D3: người gọi approve
...
- Vòng đủ của một item runner qua cổng duyệt: submit/add → runner dispatch → `proposed` → `review` → `approve` → merge → `done`, mang settlement role human, bài học câu-6, và dọn nhánh/worktree đều được assert — chứng minh bằng e2e qua binary + git thật.
...
- `AGENTS.md`'s `## fgOS Workflow` section — điểm vào của lớp hướng dẫn (P50): trỏ một phiên mới mở tới `.claude/skills/fgos-workflow/fgos-routing/SKILL.md`
- `.claude/skills/fgos-workflow/fgos-routing/SKILL.md` + `.agents/skills/fgos-workflow/fgos-routing/SKILL.md` (mirror byte-identical) — entry skill, đọc `stage` của item rồi trỏ tới đúng skill giai đoạn kế tiếp
- `.claude/skills/fgos-workflow/fgos-exploring/SKILL.md` + `.agents/` mirror — skill `làm-rõ` (stage `clarify`)
- `.claude/skills/fgos-workflow/fgos-planning/SKILL.md` + `.agents/` mirror — skill `chia-việc` (nửa đầu stage `decompose`)
- `.claude/skills/fgos-workflow/fgos-validating/SKILL.md` + `.agents/` mirror — skill `thẩm-định` (nửa cuối stage `decompose`, gác cạnh `decompose→executing`)
- `.claude/skills/fgos-workflow/fgos-executing/SKILL.md` + `.agents/` mirror — skill `thi-công` (stage `executing`, domain `coding` — str89-fgos-domain-skills)
