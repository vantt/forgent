...
- `fgos review <id>` / `fgos approve <id> [--timeout <ms>] [--acknowledge-iron-law]` / `fgos reject <id> --reason "..."` (ngoài vòng runner, gọi bởi người vận hành) — cổng duyệt PR nội bộ cho một đề xuất `awaiting-approval` đã sẵn, MỘT cổng cho cả nguồn runner lẫn pull-door; `approve` nguồn runner còn chạy phán Iron Law trước khi merge (self-improve loop STR13 Slice 3, D16/D17) — xem "Cổng duyệt PR nội bộ" dưới
...
- **Blocked when:** item không tồn tại — `validation`; item không ở
  ...
  ở trên, không git nào chạy, đề xuất giữ nguyên `awaiting-approval`. `approve` (KHÔNG
...
- **Afterwards:** `approve` nhận lại phán quyết kèm bằng chứng (đúng cờ/module
  ...
  không git nào chạy, đề xuất giữ nguyên `awaiting-approval`; `required: true` kèm cờ,
