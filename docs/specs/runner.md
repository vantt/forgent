...
- **RUL63 (`capacities.<id>.allowCrossProvider` — cổng governance chặn nội dung prompt rời hệ Claude qua backend `kind: "cli"` không rõ nguồn, mặc định hạn chế).** `resolveExecutorConfig` (`src/runner/dispatch.mjs`), sau khi giải executor thắng cuộc theo thứ tự ưu tiên `capacities.<capacityId>` > `executor` (D4, tsk-62v; rung trung gian `executors.<tier>` đã rút, tsk-in1-2 D6), kiểm thêm: nếu capacity mang `kind: "cli"` VÀ `command` CUỐI CÙNG đã giải KHÔNG nằm trong danh sách CLI-Claude đã biết (`CLAUDE_CLI_COMMANDS`, hiện chỉ `'claude'` — cố ý KHÔNG dùng chung `KNOWN_ASSISTANT_CLI_NAMES`, vì danh sách đó có `'codex'`, một CLI không phải Claude) VÀ capacity không mang `allowCrossProvider: true` tường minh → ném `RunnerConfigError` ngay tại thời điểm resolve, TRƯỚC bất kỳ dispatch nào — không bao giờ âm thầm gửi nội dung ra backend khác. Phép kiểm dựa trên `command` CUỐI CÙNG đã giải (không phải `kind` khai báo đơn thuần, không phải `provider` — nhãn hiển thị tự khai tùy ý, có thể spoof) để tránh false-positive: một capacity khai `kind: "cli"` nhưng không tự đặt `command`/`adapter` rơi thẳng về executor toàn cục, vốn thường chính là CLI của Claude, và không cần `allowCrossProvider`. Vắng mặt hoặc `false` ở `allowCrossProvider` luôn nghĩa là CHẶN (mặc định hạn chế) — không bao giờ đọc ngược thành "cho phép trừ khi đánh dấu nhạy cảm". Phạm vi hiện tại CHỈ áp dụng `kind: "cli"` — các `kind` khác (`mcp`/`http`/`skill`/`task`) chưa nằm trong phạm vi luật này (per D1-D4 tsk-32n, xem `docs/history/capacity-cross-provider-governance/CONTEXT.md`+`plan.md`).
