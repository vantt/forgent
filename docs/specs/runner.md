...
|---|-------|---------|--------|----------|---------|
| 1 | Cấu hình runner (file committed ở gốc repo) | Chính sách thực thi — LÀ CONFIG THỰC THI ĐƯỢC: ai sửa nó điều khiển tiến trình được spawn (đầu vào tin cậy) | `executor` — mẫu lệnh gọi trợ lý (thay thế {prompt}/{model} theo từng phần tử, không bao giờ qua shell), tùy chọn `adapter` (mặc định `cli-spawn`, xem CTR009 v2 dưới) · `executors` — TÙY CHỌN, bảng tier→executor riêng (cùng hình dạng `executor`); tier không khai riêng rơi về `executor` chung (STR41/D a4fe4c2b) · `models` — bảng tier→model phẳng (light/standard/heavy), HOẶC `modelPolicies` (tsk-5tm, thay thế) — bảng provider→tier→model, provider mặc định `claude`, vocab tier riêng 6 giá trị (nano/mini/standard/advanced/flagship/frontier), tự map từ tier work-item 3 giá trị qua `DEFAULT_TIER_TO_POLICY` (light→nano, standard→standard, heavy→frontier), một executor có thể override qua `rigorOverrides` riêng; `modelForTier` ưu tiên `modelPolicies` khi cả hai cùng có mặt — chỉ cần khai MỘT trong hai, không bắt buộc cả hai · `timeoutMs` — trần thời gian một worker · `confinement` — tùy chọn, `{ strict: boolean }`, kiểm soát chế độ nghiêm ngặt yêu cầu mọi capability phải có chính sách giam giữ (mặc định `false`, `fgos doctor` đưa ra cảnh báo sẵn sàng) · `confinementPolicies` — tùy chọn, bảng định nghĩa policy tuỳ biến bổ sung ngoài hai policy chuẩn `host-write-denied` và `workspace-write` · `capabilities` — khai báo capability kèm `confinement: { mode, policy?, allowInvocationOverride? }` (`mode`: `required`/`preferred`/`unconfined`) | yes | tự sinh khi vắng mặt tại đường dẫn mặc định (không kèm `--config`) — bản mặc định dò trợ lý sẵn có trên máy, dùng đúng lệnh trợ lý đó nếu nhận diện được (`claude` hôm nay), hoặc một placeholder tự-báo-lỗi nếu không (xem RUL48 (cấu hình runner tự sinh tại đường mặc định khi vắng mặt — không bao giờ đòi người tạo tay trước)/RUL48b); một `--config <path>` tường minh trỏ vào đường vắng mặt KHÔNG tự sinh, vẫn báo lỗi ngay |
| 1b | Provider-capacity inventory/state (slice 1) | Pool account cho một provider agent CLI khi provider đó có quota/capacity theo account. Inventory là machine-local secret-bearing config, không phải project policy; state/lease/quarantine là runtime-local evidence. | Inventory chỉ đọc từ `~/.fgos/config.json` tại `runner.providers.<provider>.accounts.<accountId>`; account là keyed object, không phải array. Slice 1 hỗ trợ `openai-codex` account với `label?` và `credentialSource: { kind: "codex-home", home }`. Account entry bị từ chối nếu khai placement/runtime policy (`executor`, `capability`, `model`, `modelTier`, `mutation`, `confinement`, `tool`, `fallback`, `runtimeClass`, `providerRuntimes`, ...). Project-local `.fgos/config.json` bị từ chối nếu chứa `runner.providers.*.accounts`. State ở `~/.fgos/runtime/provider-capacity/state.json`, lock ở `~/.fgos/runtime/provider-capacity/state.lock`. | no | vắng inventory nghĩa là mọi dispatch giữ hành vi cũ, không selection/lease |
...
- **RUL65b (Provider Capacity Rotator slice 1 — account selection là runtime capacity, không phải placement/fallback).** Khi một Assignment Run đã qua admission và executor thắng cuộc là `kind:"agent"` out-of-process cho provider có inventory global (slice 1: `openai-codex`), runner acquire đúng một account lease dưới lock global trước launch; `dispatch-plan.json` chỉ ghi requirement, không bao giờ ghi selected account trước admission. Ranking cố định: `openLeases` tăng dần, rồi `lastSelectedAt` tăng dần, rồi `stableHash(seed, accountId)`; sticky assignment chỉ được giữ nếu account vẫn present, healthy, non-quarantined. Lease sống theo Run lifetime và chỉ reclaim khi có proof PID-dead/run-dead, không bao giờ vì elapsed TTL. `kind:"tool"`/MCP executors (ví dụ GitNexus) bypass rotator bằng discriminator hiện hữu, dù cùng provider label.

  Selection evidence là Run-owned (`provider-capacity-selection.json`, effective execution contract/evidence) và chỉ được persist dạng redacted: provider, account id/label, lease owner, reason codes/quarantine state, `credentialProvisioned`; không persist credential home path, env values, token, `auth.json` bytes, raw digest/SHA-256. Codex bwrap provisioning dùng selected account `credentialSource` để copy `auth.json` vào private `CODEX_HOME` trước spawn; mất/không đọc/copy lỗi thì fail closed trước spawn với class credential-provisioning/config, không quarantine account, không bind-mount real Codex home, và không còn dùng `FGOS_CODEX_CREDENTIAL_HOMES` hash rotation hay executor-id allowlist làm authority placement.

  Fault classifier chỉ đọc high-confidence provider stderr, adapter outcome đã biết (`paused-limit`), hoặc structured agent JSON như `stopReason`; không scan prompt/brief/report/general stdout. Quota/rate-limit quarantine account; auth/login/token quarantine dạng manual-clear; credential provisioning/config/test/confinement/work-product failure không quarantine. Manual clear duy nhất là `fgos dispatch reconcile provider-capacity clear-quarantine --provider <provider> --account <id> --reason "<why>"` (unknown provider/account bị refuse; non-quarantined account bị refuse trừ khi `--force`). `fgos dispatch inspect --provider-capacity` và `fgos doctor` chỉ report; doctor không auto-clear auth quarantine. `readOnlyExecutorRedirects` vẫn giữ nguyên trong slice này; không có `providerFallback`, `providerRuntimes`, `providers.<provider>.models`, six-tier runtime wiring, cross-provider fallback, hay retry loop trong rotator.
 ...
 `executors.<id>.for` đổi từ 1 giá trị đơn sang mảng string không rỗng — 1 executor phục vụ nhiều capability cùng lúc; mỗi phần tử phải là 1 tên (hoặc alias) đã khai trong `runner.capabilities` (RUL11 (tùm lum, không phải nặng — gom ranh giới/contract cho rõ) trên "curated capability catalog" — xem `docs/reference/forgentx-tool-registry-configuration.md`), thay hẳn enum đóng cũ `EXECUTOR_PURPOSES` (`['judge']`, đã rút). `resolveExecutorIdForPurpose` so khớp bằng `for.includes(purpose)` thay vì `for === purpose`.
...
- **RUL68 (2 tầng dispatch — Assignment door có lease/fallback thật, purpose door không, một cầu MỘT CHIỀU; `execute --for` đã rút; quyết định giữ purpose door độc lập Work item, executor-id-consolidation).** Có 2 tầng dispatch phân biệt, không phải 2 cơ chế ngang hàng: (1) Assignment door (`executeAssignment`, `assignment-policy.mjs`'s `resolveAssignmentDispatchPolicy` — `primaryExecutor` LUÔN phải là executor id đã đăng ký thật, ném lỗi ngay nếu không) — tầng DUY NHẤT có Provider Capacity Rotator lease (RUL65b) và `opPolicy.fallbackExecutors` thật; (2) purpose door (`executeExecutorCli`/`spawnWorker`) — không lease, không fallback, theo THIẾT KẾ chứ không phải thiếu sót (RUL11 (tùm lum, không phải nặng — gom ranh giới/contract cho rõ): không dựng thêm 1 cơ chế lease/fallback song song ở tầng dưới). Cầu nối MỘT CHIỀU: `executeAssignment` tự gọi `executeExecutorCli` làm bước transport CỦA CHÍNH NÓ (sau khi đã có lease/fallback ở tầng trên); purpose door không bao giờ gọi ngược lại tầng Assignment.

  Fallback (Provider Capacity, tầng 1) từng chọn invocation MẶC ĐỊNH của candidate fallback một cách mù — khi 1 executor mang nhiều invocation khác `confinement` (executor-id-consolidation), phép chọn mù đó có thể lặng lẽ hạ cấp 1 primary có confinement xuống 1 fallback KHÔNG confinement. `selectConfinedInvocationId` (`resolve.mjs`) đóng gap này: kiểm CẤU TRÚC thuần ("candidate có invocation nào khai confinement không"), không bao giờ kiểm ngữ nghĩa policy (việc đó vẫn thuộc Confinement Authority, `confinement/authority.mjs`, không đụng tới) — primary có confinement mà fallback không tìm được invocation nào có confinement thì bị refuse thẳng, không âm thầm dispatch hở. Phạm vi CHỈ áp cho đường fallback provider-capacity; đường redirect read-only (RUL trên/Phase D) là cơ chế khác, không đụng tới ở đây. `opPolicy.fallbackExecutors` giữ nguyên `string[]` — không mở rộng thành `{executor, invocation}` (việc đó phải chạm 8+ file ở tầng khai báo Workflow/Operation, không cần thiết khi bản check cấu trúc trên đã đóng đúng gap thật).

  `execute --for <purpose>` (cửa CLI/con người duy nhất vào purpose door không lease/fallback) đã RÚT hẳn, xác nhận 0 live caller thật (chỉ có 1 proof sweep lịch sử của Confinement Authority, không phải traffic sản xuất) — `execute --for` giờ refuse thẳng, chỉ tên đúng pattern 2 phase mà mọi caller thật đã dùng: `decide --for <purpose>` giải purpose ra executor id thật trước, rồi `execute <id đó>` mới dispatch theo vị trí. `decide --for` và tham số `for` riêng của `executeExecutorCli` (dùng bởi `spawnWorker` và caller lập trình khác, không phải flag CLI) không đổi.

  Việc GỘP THẬT `spawnWorker` vào lease/fallback của `executeAssignment` (tức xoá hẳn tầng purpose door) KHÔNG làm ở đây — quyết định có chủ đích, không phải nợ kỹ thuật: purpose door là 1 primitive dispatch CỐ Ý không phụ thuộc Work item tồn tại. Work item, tới nay, chủ yếu là khái niệm QUẢN LÝ LIFECYCLE (hiện trên board), xếp lớp TRÊN dispatch, không phải điều kiện tiên quyết để dispatch được. Khi nào việc gắn Work-item lifecycle vào dispatch thật sự được xây, đó mới là điểm đúng để gộp `spawnWorker` vào `executeAssignment` — không phải trước đó.

- **RUL69 (vocab modelTier 6 mức thay 5 mức cũ; `actors[].fallbackExecutors` nối vào Provider Capacity fallback đã có, không xây retry mới; model-tier-vocabulary-and-coordination-fallback, 2026-09-17).** `MODEL_POLICY_TIERS`/`MIN_TIER_VALUES` đổi từ `lightweight|standard|creative|analytical|critical` (5 giá trị) sang `nano|mini|standard|advanced|flagship|frontier` (6 giá trị, khớp `plans/260916-account-rotator/design.md`'s "Tier vocabulary" — target architecture đã note trước đó là CHƯA implement, giờ implement thật). Mapping: `lightweight→nano, standard→standard, creative→advanced, analytical→flagship, critical→frontier` — lossless, giữ nguyên mọi model thật trong `.fgos/config.json` lẫn `~/.fgos/config.json` (chỉ đổi tên slot). `critical→frontier` (không phải `flagship`) vì gemini's `analytical`(`pro-low`)/`critical`(`pro-high`) là 2 model THẬT khác nhau — gộp chung sẽ mất phân biệt đó; claude/openai-codex/z-ai không có phân biệt này nên `frontier` trùng `flagship` ở 3 provider đó, chấp nhận được (design.md's provider table cũng vậy).
...
- **RUL70 (executor id đặt theo `providerModel`, không theo CLI binary; gộp `pi`+`codex` thành executor `openai` vì cùng provider, khác binary; executor-provider-naming, 2026-09-17).** Trước item này, executor id trộn lẫn 2 quy ước — có id theo binary (`codex`, `pi`, `agy`), có id theo alias tuỳ ý (`pi-grok`) — trong khi tầng invocation (`bin`+`account`+`via`) đã đủ sức phân biệt CÁCH gọi, và tầng executor chỉ nên trả lời câu hỏi PROVIDER nào (`providerModel`, khoá tra `modelPolicies`, và neo governance `allowCrossProvider`/`for`). Từ item này: executor id = tên provider thật (`claude`, `gemini`, `xai`, `openai`), không phải tên binary hay account. `agy`→`gemini`, `pi-grok`→`xai` là đổi tên 1-1 (không đụng `rigorOverrides`/`for`/invocation nào). `codex` (executor cũ, chỉ chạy binary `codex`) và `pi` (executor cũ, chỉ chạy binary `pi`, đã có sẵn invocation `pi-cli-tetnu`/`pi-cli-fgovn` nhắm cùng tài khoản OpenAI qua `~/.pi/accounts/{tetnu,fgovn}`) GỘP thành một executor `openai` duy nhất — hợp lệ vì `command` là field CẤP INVOCATION, không phải cấp executor (`resolve.mjs`: mỗi invocation tự khai `command` riêng), nên 2 binary khác nhau nằm chung 1 executor entry không xung đột kỹ thuật. `providerModel: "openai-codex"` (cả ở `.fgos/config.json` lẫn `modelPolicies` key lẫn `PI_EXECUTOR_DEFAULT`'s seed) đổi thành `"openai"` — Codex là TÊN SẢN PHẨM CLI của OpenAI, không phải một provider khác OpenAI, nên giữ `openai-codex` như 1 provider riêng là sai danh xưng, không phải một khoảng cách kỹ thuật cần giữ.
  ...
  Không có instance nào của 2 executor cũ mang `rigorOverrides` LỆCH nhau một cách CHỦ Ý tại thời điểm gộp (`pi`'s `{light/standard/heavy→nano}` chỉ là default còn lại từ proof-test lịch sử, chưa từng là quyết định cost-policy thật; `codex` không có `rigorOverrides` nào) — nên gộp ở đây an toàn, `rigorOverrides` bị DROP hẳn khỏi executor `openai` hợp nhất thay vì giữ lại 1 trong 2 giá trị cũ một cách tuỳ tiện. Đây không phải quy tắc chung "gộp 2 binary luôn an toàn": nếu 2 executor cũ MANG `rigorOverrides`/`for` khác nhau thật sự (một cái ưu tiên chi phí thấp, một cái ưu tiên chất lượng cao, trên CÙNG một bảng `modelPolicies.<providerModel>`), gộp chung sẽ ÂM THẦM ghi đè policy tier-lookup của 1 trong 2 — muốn giữ khác biệt đó thì tách executor bằng hậu tố (`openai-1`/`openai-2`), không bao giờ nhét vào 1 executor rồi hy vọng field nào đó "thắng" đúng ý.
...
- `.fgos/config.json`'s `runner` section (tsk-5vf; tsk-5hv xoá hẳn fallback về file flat legacy cũ — đây giờ là nguồn config DUY NHẤT, `fgos setup` là verb thực hiện move thật) — config committed (executor template + `modelPolicies.claude` nano/haiku, standard/sonnet, frontier/opus [tsk-5tm, thay `models` phẳng cũ; 6-tier vocab từ 2026-09-17] + timeoutMs); `executor.args` mang `--permission-mode acceptEdits` + `--allowedTools "Bash(git add:*),Bash(git commit:*)"` (quyền TỐI THIỂU, xem RUL6 (consumer rẽ nhánh theo mã thoát phạm trù, không bao giờ theo thông điệp)); khối `parallel` TÙY CHỌN — `maxRoots`/`maxLeavesPerRoot` (Data Dictionary #6, mặc định trong-code 4/4 khi khối vắng mặt, mọi config cũ vẫn chạy không cần sửa); `src/runner/dispatch.mjs`'s `ensureRunnerConfigForDir(dir)` — shared-file-aware: tại đường MẶC ĐỊNH (không `--config`) vắng file, gọi `detectAssistantCli` (quét PATH thuần, không spawn — `KNOWN_ASSISTANT_CLI_NAMES`) rồi viết `DEFAULT_RUNNER_CONFIG` với executor từ `SUPPORTED_EXECUTOR_TEMPLATES[detected]` khi trợ lý dò được có mẫu đã kiểm chứng (hôm nay chỉ `claude`, y hệt `DEFAULT_RUNNER_CONFIG.executor`), hoặc executor placeholder tự-báo-lỗi khi không (RUL48b) — rồi mới nạp, kèm một dòng thông báo tên file + executor được chọn (RUL48 (cấu hình runner tự sinh tại đường mặc định khi vắng mặt — không bao giờ đòi người tạo tay trước)); `bin/fgos.mjs`'s verb `discover` và `bin/fgos-runner.mjs`'s vòng chính đều gọi `ensureRunnerConfigForDir` cho nhánh đường mặc định, `loadRunnerConfig` thẳng cho nhánh `--config` tường minh; đọc qua `mergeWithGlobalConfig` trước khi tách phần `runner` (project luôn thắng `~/.fgos/config.json` theo key, tsk-5vf)
...
6. **Group cognition không đổi ranh giới đã có:** `fgos-panel` là surface natural-language chọn một use-case preset theo bảng canonical tại `docs/architect/agent-coordination/architecture/group-thinking-trigger-surface.md`; `fgos-group-thinking` vẫn là cổng pack mỏng nhận protocol id tường minh từ surface/operator trước `fgos coordination run/show`, điều phối nhiều research contribution nhưng không thay thế hay nhân bản `fgos-researching`. Preset không phải routing identity/capability mới, không định nghĩa protocol semantics, và không pin provider/model/executor/tier; per-actor override hợp lệ của team-cognition vẫn tách biệt khỏi capability annotation ở planning time.
7. **P3 static harness (read-only, không guard):** `src/report/capability-plan-lint.mjs` lint plain text theo đúng convention `- unit: ... / capability: <name>` ví dụ trong `planning-capability-awareness.md` — check syntax hợp lệ, capability đã đăng ký (hoặc đánh dấu `unresolved` rõ ràng), và không có `executor`/`provider`/`model`/`tier` pin trong CHÍNH block đó (không quét toàn file, tránh false-positive trên văn xuôi nói về provider/model như một khái niệm). Không có plan.md nào trong repo dùng convention này chưa nên module chỉ là harness sẵn sàng, không được wire vào `fgos doctor` hay bất kỳ gate tự động nào — execution-boundary guard theo P3 vẫn chờ bằng chứng dogfood non-compliance thật.
8. **P4 observability:** `logExecutorDispatch`/CLI `log` subcommand (`src/runner/dispatch/cli.mjs`) nhận thêm field optional `capability`/`mechanism`/`tier`/`fallbackReason`/`outcome` vào `executor.dispatch` event — additive, mọi caller cũ bỏ qua các field này vẫn ghi payload y hệt trước (null). `outcome` hoàn thiện đường đọc `classifyDispatchConfidence` (`src/report/dispatch-confidence.mjs`) đã có sẵn cho tier "reported" nhưng chưa từng có writer nào điền. `executor-dispatch-fallback.md` Step B.5 cập nhật ví dụ lệnh `log` để truyền các field mới khi biết.
...
### Confinement Authority — một cửa kiểm soát giam giữ runtime (executeThroughConfinement)

Mọi lần dispatch đưa agent ra ngoài process (bao gồm `spawnWorker`, `executeExecutorCli`, `herdr-spawn`, và `fgos coordination run`) đều bắt buộc đi qua một cửa runtime Confinement Authority duy nhất (`src/runner/dispatch/confinement/authority.mjs: executeThroughConfinement`) trước khi bất kỳ executor adapter nào được phép spawn hoặc giao việc cho agent:

1. **Một cửa runtime duy nhất:** Không adapter nào được gọi trực tiếp bởi production call site; kiểm tra AST/import (`test/architecture.test.mjs`) bảo đảm tính bất biến này.
2. **Fail-closed:** Policy `mode: "required"` từ chối trước spawn (refuse) đối với mọi điều kiện thiếu backend (`confinement-backend-missing`), backend disabled (`confinement-backend-disabled`), control không hỗ trợ (`confinement-unsupported`), probe cũ/hỏng (`confinement-probe-failed`), grant không hợp lệ (`confinement-grant-invalid`), unsatisfied resource need (`confinement-need-unsatisfied`), hoặc plan mismatch (`confinement-plan-mismatch`).
3. **Bypass pairing:** Mọi executor khai `permissionMode: "bypass"` bắt buộc phải đi kèm full confinement (`home: "private"`, `session: "isolated"`, `workspace: "own"`), được kiểm tra qua hàm dùng chung `evaluateBypassPairing` ở cả Authority lẫn `herdr-round.mjs`.
