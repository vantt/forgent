...
area: work-state
updated: 2026-07-16
sources: [phase-1-state-layer, phase-1-review-fixes, phase-2-routing-s1, phase-2-routing-s2, phase-3-compound-learning-s1, phase-3-compound-learning-s2, phase-3-compound-learning-s3-closeout, async-human-gate, stage-intake, stage-clarify]
decisions: [9ac6ca50, 0790031c, 451ca088, fd17309a, 55ad2f9f, feed7428, 1a80b4d3, 65c642a8, 9f6b52c8, 9a19eea5, 96a65365, a7c099af]
coverage: full
---
...
| # | Element | Meaning | Values | Ghi khi nào |
|---|---------|---------|--------|-------------|
| S1 | loại ngã-ngũ (kind) | Loại điểm quyết-xong | `clarify-pass` — context-discovery cho qua (clarify→executing) · `answer` — người trả lời một câu hỏi đang chờ · `close` — item tới `done` (qua cả hai lối vào) | lúc chính ngã-ngũ đó xảy ra |
...
- `bin/fgos.mjs` — verb `check`: đọc `listWork(dir).outcomes`, in predicted-vs-actual; cộng thêm mục friction (đọc `view.frictions`, đếm theo lớp + cap 5 record gần nhất), mục settlement (đọc `view.settlements`, đếm theo kind+actor + cap 5), mục học (đọc `view.learnings`, cap 5), và nhắc item trạng thái cuối thiếu outcome — tất cả read-only, không sự kiện mới; tín hiệu entropy-trend + seal-digest trên cùng `check` — xem spec Runner; verb `ask`/`answer` gọi `putInAwaiting`/`answerAwaiting`; `list` mang `view.gates` (câu hỏi hiện ra không cần formatter mới); verb `submit` — gọi `classify.mjs` (deriveTitle/classify/generateId) + `envelope.mjs` (wrapEnvelope) rồi `addWork` sẵn có, KHÔNG cửa ghi mới, gắn `stage:'clarify'`; verb `discover` — gọi thẳng `resolveDiscovery`
- `src/runner/loop.mjs` — `runOnce`: NGAY SAU startupReap, TRƯỚC vòng dispatch executing, quét mọi item `stage==='clarify' && status==='todo'` (không đọc `item.mode`) và gọi `resolveDiscovery` cho từng item — lưới đỡ R19; mọi `moveWork` runner tự ghi (claim/propose/park) gọi kèm `actor:'runner'`; sweep gọi `resolveDiscovery(..., 'runner')`
...
- Test: `npm test` (394 test; e2e tại `test/e2e/rebuild-determinism.test.mjs` + `test/e2e/runner-loop.test.mjs` — bao gồm 3 kịch bản stage-clarify (verdict pass/unclear/rác) chạy qua binary thật; round-trip cổng chờ-người tại `test/state/awaiting.test.mjs` + e2e CLI tại `test/cli/fgos.test.mjs` bao gồm `submit`/`discover`/settlement/học; unit tại `test/intake/{classify,discovery}.test.mjs` + `test/state/{envelope,stage,store}.test.mjs`; entropy-trend tại `test/report/entropy.test.mjs`; benchmark ngoài suite (F4, expected-delta khai trước run) tại `docs/history/phase-3-compound-learning/reports/f4-benchmark.md`)
