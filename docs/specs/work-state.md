...
area: work-state
updated: 2026-07-16
sources: [phase-1-state-layer, phase-1-review-fixes, phase-2-routing-s1, phase-2-routing-s2, phase-3-compound-learning-s1, phase-3-compound-learning-s2, async-human-gate, stage-intake]
decisions: [9ac6ca50, 0790031c, 451ca088, fd17309a, 55ad2f9f, feed7428, 1a80b4d3, 65c642a8, 9f6b52c8]
coverage: full
---
...
- `fgos submit "<mô tả tự do>" [--async|--unattended]` → khai một work item từ một câu mô tả văn xuôi duy nhất — id, title, kind, risk, tier đều TỰ SUY (không cần người submit tự đặt); `verify` nhận placeholder cố định chờ bổ sung sau; kết quả in ra bọc trong một phong bì máy-đọc chuẩn (xem "Phong bì output" dưới)
...
- **Afterwards:** kết quả in ra là work item vừa tạo, bọc trong phong bì máy-đọc
  (xem dưới); item xuất hiện trong `list`/`ready` như mọi item khác được tạo
  qua `add`.
...
- **R17 (mode ghi lại, chưa gây hành vi).** Trường `mode` do `submit` ghi lại chế độ đã dùng khi tạo item, nhưng bản thân bước `submit` không đọc/không hành động dựa trên nó — không có gì tự động đậu chờ người dù `mode:'async'`; hành vi đó thuộc một stage tương lai chưa xây (per D6 stage-intake / 9f6b52c8). Cho tới lúc đó, `mode` là dữ liệu quan sát được nhưng không có hệ quả.
...
- Test: `npm test` (304 test; e2e tại `test/e2e/rebuild-determinism.test.mjs` + `test/e2e/runner-loop.test.mjs`, chạy binary thật trong tmp dir; round-trip cổng chờ-người tại `test/state/awaiting.test.mjs` + e2e CLI tại `test/cli/fgos.test.mjs` bao gồm `submit`; unit tại `test/intake/classify.test.mjs` + `test/state/envelope.test.mjs`)
