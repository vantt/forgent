...
decisions: [9ac6ca50, 0790031c, 451ca088, fd17309a, 55ad2f9f, feed7428, 1a80b4d3, 65c642a8, 9f6b52c8, 9a19eea5, 96a65365, a7c099af, 43f257ae, 44936500, e1218b22, 6f2cbc47, a30a3d3c, 1359ab5e, f1715488, 8788e9bb, cfae0120, 396d9d9e, 2e92b7a5, 5a6900b2, b28487af, 2ae492d8, 76b7a36b, 8d04bba3, 1cd895e1, 38160a70, a2146274]
coverage: full
---
...
| 5 | deps | Các id item phải xong trước; mọi id phải tồn tại, cấm tự trỏ; "epic" chỉ là một item thường được deps trỏ vào. **Bất biến phi-chu-trình (per work-graph-intelligence S1):** đồ thị `deps` không bao giờ được phép khép vòng — cửa ghi duy nhất (`addWork`/`editWork` trong `store.mjs`) chặn MỌI lần ghi (thêm mới hoặc sửa) mà kết quả sẽ tạo một chu trình (A→B→A hoặc dài hơn), ngay sau bước kiểm tồn tại, TRƯỚC khi sự kiện được ghi; lỗi thuộc phạm trù `validation` (mã thoát 4), dùng đúng một đường kiểm tra (`src/state/dep-graph.mjs`), không có đường kiểm chu trình thứ hai. Trước cell này, `editWork` có thể vá `deps` tạo chu trình A↔B mà lọt qua âm thầm (`validateDeps` chỉ kiểm tồn tại) — lỗ hổng đó nay đã đóng | danh sách id | yes (rỗng được) | `[]` |
