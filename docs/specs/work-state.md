...
| F6 | phân loại tài liệu (docType) | Nhãn Diataxis TÙY CHỌN, cùng nghĩa và cùng khuôn với O10 của bản ghi outcome (trực giao với type-axis kỹ sư, kiểm khi có mặt, vắng/`null` = chưa gắn) — đi ké payload thô của sự kiện friction, sống sót replay không đổi cơ chế (per D5/D6) | `tutorial` / `how-to` / `reference` / `explanation` | tùy — lúc bên ghi capture cung cấp |
...
- Nội dung tổng hợp thật của Compound-learn (phán/kỹ năng tổng hợp capture thành bài học hoặc tài liệu người-dùng cuối) chưa tồn tại — `fgos compound` hôm nay chỉ chuyển stage, không tự sinh bản ghi tổng hợp nào. Chiều phân loại Diataxis nay đã có TRƯỜNG capture để mang nhãn (`docType`, tùy chọn, RUL52) và `fgos check` đã hiển thị nó, NHƯNG bên SẢN XUẤT tự động cấp giá trị nhãn (kỹ năng tổng hợp chạy ở stage `compound-learn`) vẫn chưa build; lớp nội dung + nối producer đó là slice sau của compound-learn-enduser-docs (per D4-D8 compound-learn-enduser-docs).
