...
| # | Element | Meaning | Values | Ghi khi nào |
|---|---------|---------|--------|-------------|
| S1 | loại ngã-ngũ (kind) | Loại điểm quyết-xong | `clarify-pass` — context-discovery cho qua, item RỜI stage ĐẦU CHUỖI của domain nó (`discovery` với `coding`; đích là `planning` hay `exploring` đều ngã-ngũ y hệt). Tên kind là nhãn DI SẢN đã ghi vào nhật ký, không phải tên stage — xem RUL27 · `answer` — người trả lời một câu hỏi đang chờ · `close` — item tới `done` | lúc chính ngã-ngũ đó xảy ra |
...
mới khai item. Cửa khai từ chối `stage: clarify` trên bất kỳ item mới nào;
toàn bộ item từng đỗ trên tên đó đã được di trú THẬT sang chuỗi mới (khác
`decompose` dưới, vốn được giữ lại làm bí danh), nên không còn gì mắc kẹt.
...
- **Runs when:** người/agent gọi `fgos discover <id> [--verdict clear|unclear]`
...
- **RUL27 (settlement `clarify-pass` theo cạnh RỜI stage đầu chuỗi, không theo đích cụ thể).** Bản ghi settlement kind `clarify-pass` ghi khi item RỜI stage ĐẦU CHUỖI của domain nó — với `coding` hôm nay là `discovery` — bất kể đích là `planning` (verdict đủ rõ) hay `exploring` (verdict chưa đủ rõ). Khóa theo cạnh RỜI, không theo cạnh ĐẾN, để việc chèn stage mới ở giữa không làm câm bản ghi settlement đã có (per D2 stage-decompose / 43f257ae); cái đổi khi `clarify` rút là stage nào giữ vai "đầu chuỗi", không phải hình dạng của luật. Tên `clarify-pass` GIỮ NGUYÊN như một nhãn di sản: nó là giá trị đã ghi vào nhật ký append-only, không phải một tên stage, nên đổi tên sẽ vô hiệu các bản ghi cũ mà chẳng được gì. Hệ quả cần biết: các hop SAU đó (`exploring → planning`, `planning → executing`) KHÔNG sinh settlement — một item đi đường vòng qua `exploring` vẫn chỉ có đúng một bản ghi `clarify-pass`, ghi tại cạnh `discovery → exploring`.
