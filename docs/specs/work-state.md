...
- **Blocked when:** không truyền `id` và frontier rỗng — `validation`; `id` truyền một id không tồn tại — `validation`; `id` truyền một item đã bị cầm/đỗ/kẹt — rơi thẳng xuống CAS của cạnh chuyển trạng thái, báo `conflict` thật (mã 3) — HỆT `take`, không có cờ `--actor` (actor LUÔN `session`, không đọc/chấp nhận cờ đó).
