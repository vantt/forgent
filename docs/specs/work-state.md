...
area: work-state
updated: 2026-07-17
sources: [phase-1-state-layer, phase-1-review-fixes, phase-2-routing-s1, phase-2-routing-s2, phase-3-compound-learning-s1, phase-3-compound-learning-s2, phase-3-compound-learning-s3-closeout, async-human-gate, stage-intake, stage-clarify, stage-decompose-s1, stage-decompose-s2, pr-lifecycle-s1, install-coexistence, discovery-context, worker-execution, fan-out-parallel, human-rounds, work-item-verb-surface, base-workflow-model-s1, self-improve-loop]
decisions: [9ac6ca50, 0790031c, 451ca088, fd17309a, 55ad2f9f, feed7428, 1a80b4d3, 65c642a8, 9f6b52c8, 9a19eea5, 96a65365, a7c099af, 43f257ae, 44936500, e1218b22, 6f2cbc47, a30a3d3c, 1359ab5e, f1715488, 8788e9bb, cfae0120, 396d9d9e, 2e92b7a5, 5a6900b2, b28487af, 2ae492d8, 76b7a36b, 8d04bba3]
coverage: full
---
...
Hôm nay tồn tại ĐÚNG MỘT domain, `coding` — tái tạo byte-for-byte danh sách
stage / step-mapping / cạnh chuyển-stage đã có từ trước (xem "Giai đoạn
Làm-rõ"/"Giai đoạn Chia-việc" trên). Domain thứ hai (minh họa: marketing) và
việc nối nó vào vòng thi công thật hay cửa CLI (`submit --domain`) là
backlog CHƯA làm (xem Open Gaps) — mô hình hôm nay chỉ chứng minh domain
`coding` chạy được qua đúng một sổ đăng ký chung, chưa chứng minh domain
thứ hai chạy trên cùng sổ đó.

Item KHÔNG mang trường `domain` (100% item hôm nay) đọc ra `coding` — mặc
định lazy, cùng khuôn `stage`'s D8. Một giá trị `domain` không nhận diện
được tới điểm đọc nóng của vòng dispatch (bộ lọc frontier, vòng tự hành,
bảng chuyển-stage) KHÔNG BAO GIỜ làm vỡ đường đó: cả ba rơi về `coding` kèm
một cảnh báo, không throw — khác với lúc KHAI (`validateWork`), nơi một giá
trị `stage` không hợp với domain của item vẫn bị từ chối `validation` như
trước (có chủ đích: một bên là đường nóng không được vỡ, một bên là cửa
khai chỉ chạy một lần, sai thì báo ngay).
...
- **R35 (domain — chiều thứ ba chi phối bộ stage, song song status/stage).** Một domain khai đúng ba thứ: danh sách stage có thứ tự, step-mapping (bước nào trong 5 bước base-workflow mỗi stage thỏa), và cạnh chuyển-stage hợp lệ riêng của nó — domain KHÔNG BAO GIỜ chi phối bảng chuyển-status (`fsm.mjs`), tách bạch tuyệt đối khỏi `status`. Hôm nay tồn tại đúng một domain, `coding`, tái tạo byte-for-byte mọi giá trị stage/step-mapping/cạnh-chuyển đã có; domain thứ hai và việc nối nó vào vòng thi công/CLI thật (`submit --domain`) là backlog chưa làm (per D1-D3 base-workflow-model / 2ae492d8; xem Open Gaps). Item vắng `domain` (100% item hôm nay) đọc ra `coding` — mặc định lazy, cùng khuôn `stage`'s D8. Một giá trị `domain` lạ tại các điểm đọc nóng (frontier/vòng tự hành/bảng chuyển-stage) fail-safe về `coding` kèm cảnh báo, không throw.
...
- Domain thứ hai thật (minh họa: marketing) + việc nối nó vào vòng thi công thật (`runner/dispatch.mjs`) và cửa CLI (`submit --domain`) — Slice 1 (base-workflow-model) chỉ dựng sổ đăng ký + retrofit domain `coding`; Slice 2 (deferred) mới thêm domain thứ hai chạy end-to-end (per D4 base-workflow-model / 2ae492d8; backlog P18 tiếp tục).
