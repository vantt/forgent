...
| 4 | status | Trạng thái vòng đời | `todo` — chưa bắt đầu · `doing` — đang làm · `blocked` — kẹt, hai chiều với todo/doing · `done` — xong, TERMINAL: vào duy nhất từ `doing`, không bao giờ ra | yes | `todo` |
...
- **Blocked when:** (a) cạnh chuyển không có trong bảng — `todo→doing`, `doing→done`, `todo/doing→blocked`, `blocked→todo/doing` là toàn bộ cạnh hợp lệ — trả `precondition` (mã 2); (b) trạng thái thực khác `--expect` — trả `conflict` (mã 3). Cả hai trường hợp KHÔNG ghi sự kiện nào.
...
- `src/state/store.mjs` — chủ ghi duy nhất (append event → update view)
