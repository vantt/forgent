...
Confirmed live, repeatedly: `readGateBypassLevel` returns a real
configured level in this repo, and `canAutoApprove`/
`canAutoApproveValidate` legitimately return `true` for real items —
...
already said, rather than inventing new behavior: the frontmatter now
reads "auto-approves when the repo's configured gate-bypass level covers
it, otherwise pauses for real human approval," and the hard rules bullet
now reads "Never bypass a gate beyond what its own dev-skill already
