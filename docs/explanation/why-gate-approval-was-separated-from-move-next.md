...
verify must be proposed and recorded by `fgos-exploring`/`fgos-planning`
...
- **D11**: the structured approve record needed three parallel fields,
  ...
  `gates[id]`, since `fgos-exploring`/`fgos-planning`/`fgos-validating`
  each have their own gate. Each field is `{actor, at, verify}` —
  deliberately distinct from `fgos-validating`'s own feasibility-matrix
...
retrofit. An initial split proposal put `src/intake/decompose.mjs` in
