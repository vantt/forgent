---
type: tutorial
title: Walking a heavy fgOS item through clarify, a 3-child split, and done
tags: []
timestamp: 2026-07-29T03:44:15.000Z
source_capture_ids: [tsk-3wr]
---
# Walking a heavy fgOS item through clarify, a 3-child split, and done
...
(`fgw/tsk-3wr`) — the right choice for `clarify`/`decompose` work, since
...
## 2. Clarify: lock decisions before shaping anything

At stage `clarify`, scout the real codebase for evidence, then lock
...
This is the step that actually moves `clarify -> decompose` — writing
...
At `decompose`, `fgos-planning` writes `plan.md`: the mode (this item was
`standard`, since 67% of the suite needed judgment-level review despite
...
Calling `fgos discover tsk-3wr` again here doesn't just rubber-stamp the
plan — it runs the engine's own `judgeDecompose`, which can disagree.
Here it did: it proposed a 3-child split (rename / dedup / measure) where
the human-approved plan had assumed one batched item. That's a real
...
The second `discover` call is what actually writes the three children
...
Before any child is worked, `fgos-validating` scores a reality-gate
matrix — every claim needs a real command's real output as evidence, not
plausibility language. For this item that meant actually running the new
...
## 6. Compound-learn: synthesize before `done`

A coding-domain item cannot reach `done` straight from `proposed` — it
must pass through `compound-learn` first, proven by a real FSM guard, not
a convention:
...
— it re-verifies on `main` and closes straight to `done`, since the work
was already committed directly there; no branch merge step is needed the
way a `pick`ked item's would be.

## 7. Watch for your own doc changing what your own tests assume

`tsk-3wr-3`'s compound-learn step wrote the very first real file into
...
## 8. The parent closes itself once every child is done

Once all three children reach `done`, the frontier's lineage filter
re-admits the parent (`tsk-3wr` was root-blocked by its own open children
until then — a root is included once every child reaches `done`, checked
independently of the root's own `deps`). Claim it, run its own
(corrected) `verify` as a final capstone check, `return`, compound-learn
this very tutorial, and `approve` — the same six steps as any other item,
just run last.
