...
| Workspace local `fgos` can enter the Rust host through activation. | partial | `test/rust-host/fgctl-init.test.mjs`, `scripts/ci-external-consumer.sh`, `docs/platform/host-invocation-routing/verification/r1-rust-host-proof.md` | Keep as partial until release assets and public/default posture are decided; current proof covers local fixture and external-consumer asset flow where `version --runtime-json` reports `host: "rust"` and the activated `artifactDigest`. |
...
| Legacy npm/Node install remains compatibility path. | implemented legacy deprecated | `package.json`, `bin/fgos.mjs`, `bin/fgos-runner.mjs`, `test/install-packaging.test.mjs`, `README.md`, `docs/platform/packaging-distribution/verification/install-and-release-proof.md` | Rust host is the default runtime path for activated workspace installs. Legacy Node fallback is deprecated immediately as public/default posture and may remain only as an explicit escape hatch with warning/log/proof marker during a short compatibility window. Exact removal date or release milestone still requires release-owner approval. |
