...
| Host-neutral skill intent ids map to host-native triggers. | implemented | `src/setup/skill-wrappers.mjs` (`mapSkillIntentToHostTriggers`, `discoverCanonicalSkills`), `test/setup/skill-wrappers.test.mjs` | Keep host trigger vocabulary synchronized across new domain skills and adapter projections. |
| Gemini receives a native extension/package target for fgOS skills. | implemented | `src/setup/skill-wrappers.mjs` (`generateGeminiSkillPackage`), `test/setup/skill-wrappers.test.mjs` | Wire into distribution release packaging and doctor registration when Gemini extension install is published. |
