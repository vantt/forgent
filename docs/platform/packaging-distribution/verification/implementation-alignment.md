...
| Legacy npm/Node install remains compatibility path. | implemented legacy deprecated | `package.json`, `bin/fgos.mjs`, `bin/fgos-runner.mjs`, `test/install-packaging.test.mjs`, `README.md`, `docs/platform/packaging-distribution/verification/install-and-release-proof.md` | Coordinator decision (2026-09-15) confirms this posture for the preview release: Rust host is the default runtime path for activated workspace installs and external installs. Legacy Node fallback is deprecated immediately as public/default posture and remains only as the settled, explicit escape hatch with warning/log/proof marker during a short compatibility window. Exact removal date or release milestone still requires release-owner approval and remains undecided. |
