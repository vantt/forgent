...
| Workspace `.fgos/distribution.json` is the tracked team/project runtime pin. | `runtime-identity-and-activation.md`, `scope-map.md` | `contracts/distribution-pin.md`; `contracts/repository-runtime-layout.md` | preserved partial | Freeze exact schema once pin resolution is fully implemented. |
| Workspace `.fgos/installation/activation.json` is the ignored per-workspace ready binding. | `runtime-identity-and-activation.md`, `migration-map.md` | `contracts/activation-binding.md`; `contracts/repository-runtime-layout.md`; `verification/implementation-alignment.md` | preserved partial | Extract exact activation schema from Rust implementation and tests. |
...
| Release manifest identity is `artifactDigest`, not archive checksum. | `runtime-identity-and-activation.md`, `migration-map.md` | `contracts/release-manifest.md`; `verification/implementation-alignment.md` | preserved implemented | Promote to JSON schema if external consumers read the manifest directly. |
...
| Canonical skill source lives under component/domain authority; host skill folders are projections/adapters. | Current skill-source scan and distribution discussion | `contracts/skill-package-distribution.md`; `verification/implementation-alignment.md` | preserved implemented/planned | Move `fgos-code-panel` to coding domain in its own implementation phase. |
| `fgos-code-panel` is a coding-domain application surface over coordination/group-thinking, not a group-thinking special case. | Current design discussion | `contracts/skill-package-distribution.md`; `verification/implementation-alignment.md` | preserved planned | Prove by moving source to `domains/coding/skills/fgos-code-panel/` and retaining compatibility projections. |
...
| --- | --- | --- |
| Exact `activation.json` schema is not yet frozen in promoted docs. | External tools and doctor/repair need a stable activation record. | Packaging-distribution contracts. |
| Exact `.fgos/distribution.json` schema and source selection precedence need final proof. | Team/project runtime pins are the boundary between branch policy and machine installation. | Packaging-distribution contracts plus Rust distribution tests. |
