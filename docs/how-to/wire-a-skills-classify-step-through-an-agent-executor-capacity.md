...
1. **Add the capacity to `.fgos-runner.json`** — `kind: "cli"`,
   ...
   If the capacity might route content to a different model family than
   the rest of the runner (cross-provider), also add a `sensitiveData`
   boolean now — even before any code reads it — so the first
   cross-provider capacity isn't shipped with zero governance vocabulary
   at all (real precedent: `sensitiveData: false` on the same entry,
   since the routed content there is only a short free-text ask, never
   repo/code content).
