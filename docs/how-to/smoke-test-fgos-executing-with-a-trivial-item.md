---
type: how-to
title: How to smoke-test fgos-executing with a trivial verify=true item
tags: []
timestamp: 2026-07-29T00:00:00.000Z
source_capture_ids: [str89-case-study-executing]
---
# How to smoke-test fgos-executing with a trivial verify=true item

Use this when you want to confirm `fgos-executing`'s own claim-to-proposed
...
1. Add a `chore`-kind item with `verify` set to the no-op string:
   ...
   `"STR89 case study: exercise fgos-executing"`, `kind: "chore"`,
   `risk: "low"`, `verify: "true"`, no `description` field at all:

   > `{"type":"work.add","payload":{"id":"str89-case-study-executing","title":"STR89 case study: exercise fgos-executing","kind":"chore","status":"todo","deps":[],"risk":"low","refs":[],"verify":"true","tier":"standard"}}`
   > — real `work.add` capture, id `str89-case-study-executing`, `.fgos/events.jsonl:97`

2. Claim/pick it and let `fgos-executing` run. With `verify: "true"` there
...
`fgos-executing` without the skill needing a retry or hitting a verify
...
- `fgos-executing` skill — the skill this smoke test exercises.

## Document history (compound-learn capture linkage)

This doc's path
(`docs/how-to/smoke-test-fgos-executing-with-a-trivial-item.md`) is linked
...
>   "actual": {"outcome":"proposed","passed":true,"attempts":1,"errorClass":null,"aheadCount":1},
>   "docType": "how-to",
>   "docPath": "docs/how-to/smoke-test-fgos-executing-with-a-trivial-item.md"
> }
> ```
