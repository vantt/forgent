---
type: how-to
title: How to wire a headless function through an agent-executor capacity
tags: []
...
- `spawnAttempt(cfg, model, prompt, tier, scoutCapture, capacityId, fgosDir)`
  passes `capacityId`/`fgosDir` straight into
  `resolveExecutorCommand(cfg, { prompt, model, tier, capacityId, fgosDir })`.
...
- `runJudgeExecutor(cfg, model, prompt, stricterPrompt, scout, capacityId,
...
1. **Find every function in the call chain between the public entry point
   ...
   `capacityId` (and `fgosDir`, if the caller has a `.fgos/` dir in scope —
...
3. **Hardcode the capacity id at the call site, not as a caller-supplied
...
5. **Test the precedence and propagation directly against the function**,
   ...
   - the same `kind: "cli"` capacity with `fgosDir` omitted resolves
