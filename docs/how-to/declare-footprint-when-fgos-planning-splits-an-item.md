# How to declare `--footprint` when `fgos-planning` splits an item

Use this when `fgos-planning`'s own "Decide the split" step is creating
...
`fgos-planning`'s split step used to create children with no
`--footprint` at all — confirmed by grep, the word never appeared
anywhere in `fgos-planning/SKILL.md`. This is a different gap from
...
childs-footprint.md`): a manual split done via `fgos-planning` produced
...
was purely that `fgos-planning`'s own instructions never told a session
to fill it in.
...
`fgos-planning`'s own instructions, not to `fgos add`'s schema or to
