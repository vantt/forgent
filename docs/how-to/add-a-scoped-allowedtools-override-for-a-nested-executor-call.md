...
> had itself already migrated to `runner.capacities.judge` before this,
...
> site: give that call site its own `capacities.<capacityId>` entry with
> an explicit `command`/`args` (the same executor-block shape), and pass
> that id as `resolveExecutorCommand`'s `capacityId` — see `docs/how-to/
> wire-a-headless-function-through-an-agent-executor-capacity.md`. Kept
