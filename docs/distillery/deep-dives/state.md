...
topic: state
date: 2026-07-13
redived: 2026-07-15 (delta beegog af4840c→55cf3a4: state-shape thành đa-pipeline + producer-not-settable; bee rời single-writer); 2026-07-21 (delta beegog 94b4a31→05a131f: ghi vào state nay khoá đặt-tên chéo tiến trình; bee tự có một dạng "durability gate" như symphony; cell store tách hot/archive)
based_on: [beegog@05a131f, repository-harness@9cc306d, symphony@2f0b257, marketing-cockpit@588d800]
entries: [beegog:state-vs-log-two-physics, beegog:phase-machine-cli-owned, beegog:cell-status-lifecycle, beegog:policy-vs-ops-split, beegog:docs-history-per-feature, beegog:lane-scoped-pipelines, beegog:cross-session-atomic-claims, beegog:chain-integrity-guard-tail, beegog:store-lock-named-mutex, beegog:worktree-merge-staged-verify-gate, beegog:cell-archiving-transaction, repository-harness:durable-sqlite-layer, repository-harness:changeset-event-sourcing, repository-harness:story-status-single-door, repository-harness:epoch-fence-migration-guard, repository-harness:policy-vs-durable-separation, symphony:run-and-queue-state-machine, symphony:run-artifact-durability-split, symphony:changeset-content-sha-immutability, symphony:board-state-precedence-derivation, marketing-cockpit:four-memory-types, marketing-cockpit:procedural-memory-reinforcement, marketing-cockpit:four-zone-storage-separation, marketing-cockpit:task-signal-state-machines]
---

# Deep-dive: state

> **Re-dive delta beegog 94b4a31→05a131f (2026-07-21):** ba mảnh mới chạm store vật lý + concurrency + durability, kết luận 4-tầng KHÔNG đổi. (1) `beegog:store-lock-named-mutex` (entry MỚI) — nền CHUNG mà `cross-session-atomic-claims` (đã cite) và `state-vs-log-two-physics` (đã cite) đứng lên: `withStoreLock(root, name, fn)` khoá đặt-tên chéo tiến trình (lockfile O_EXCL, staleness HAI TẦNG — 30s là ứng viên, cướp còn đòi pid chủ chết hoặc trần cứng 1h) bọc TOÀN BỘ đọc-kiểm-ghi của cells/claims/reservations/state. Đây là câu trả lời cho câu hỏi "concurrency trên state" mà bản redive @55cf3a4 mới nêu hiện tượng (atomic claim) — nay có PRIMITIVE chung một tầng dưới cả claims lẫn state. (2) `beegog:worktree-merge-staged-verify-gate` (entry MỚI) — bee lần đầu có một dạng **durability gate** giống hệt "chạy xong ≠ đã merge ≠ đã bền" mà symphony đóng góp riêng trước đây: `bee worktree merge` stage bằng `git merge --no-commit`, chạy verify trên cây CHƯA commit, chỉ commit khi xanh; verify đỏ → abort + bằng chứng main-byte-untouched, không bao giờ để lại merge commit không đảo được. Đóng đúng khoảng trống Bottom Line từng ghi ("symphony đóng góp riêng biệt: durability tách khỏi state-shape") — bee KHÔNG còn là ngoại lệ duy nhất thiếu khái niệm này, dù cơ chế khác symphony (git-merge-transaction thay vì artifact-tier). (3) `beegog:cell-archiving-transaction` (entry MỚI) — store JSON zero-dep của bee (đã cite là điểm khác biệt so SQLite) nay tách hot-scan (`.bee/cells/*.json`) khỏi archive (`.bee/cells/archive/<feature>/`) khi feature đóng, giao dịch có journal + preflight chống ghi đè; lý do là SCALE THẬT (một lần archive 108 feature/356 cell) — cùng động cơ khiến harness chọn SQLite ngay từ đầu, nhưng bee giải bằng file-tier thay vì đổi engine.

> **Re-dive delta beegog 55cf3a4→94b4a31 (2026-07-18):** trục state-shape/store thêm chiều. (1) `beegog:independent-feature-worktrees` — mỗi worktree bootstrap `.bee/` store RIÊNG (phase/gate độc lập) + `.gitattributes` union-merge cho các file log-tier (decisions/backlog/review-candidates.jsonl): cơ chế đa-pipeline THỨ HAI ngoài `lane-scoped-pipelines` đã cite, cập nhật ô "Store vật lý"/"Số pipeline state". (2) `beegog:performance-log-cross-project-matrix` — physics store khung log-vs-state chưa phủ: log global/cross-project/per-machine append-only (vs per-feature-log / per-area-state của bee) — một dòng ở "Store vật lý". (3) `beegog:computed-parallel-schedule` — chứng cứ phụ cho hội tụ #3 "state = sản phẩm một hành động thật, derived-không-lưu", không phải kết luận mới. Kết luận memory-layer (typed vs 2-physics) KHÔNG đổi.

> **Re-dive delta beegog af4840c→55cf3a4 (2026-07-15, theo yêu cầu — CÓ dịch chuyển 2 điểm):** fsh-1..13 + chain-integrity đụng thẳng trục (1) state-shape và mặt concurrency. (1) **state-shape thêm hai dạng mới:** *đa-pipeline song song* — `beegog:lane-scoped-pipelines` cho mỗi feature một pipeline riêng (phase/gate cô lập), `resolvePipeline` giải state qua lane của phiên trước; state không còn là một record toàn cục. Và *producer-not-settable* — `beegog:chain-integrity-guard-tail`: phase `compounding` KHÔNG gán-được-value, chỉ SINH bởi `scribing-run` thật; nâng "transition = API có precondition" thành dạng mạnh hơn "state = sản phẩm của hành động thật, không phải giá trị ghi vào". (2) **bee RỜI single-writer:** `beegog:cross-session-atomic-claims` (O_EXCL+TTL/heartbeat) + cross-session hold deny — bảng So sánh ô "Concurrency trên state: beegog = 1-writer (không cần)" nay SAI, đã sửa. Concurrency chi tiết vẫn ở `routing` §tầng-1 (đã cập nhật cùng ngày). Kết luận memory-layer (typed vs 2-physics) KHÔNG đổi. Ngấm vào Bottom Line, beegog §state-shape, So sánh, hội tụ #3, Portable ideas.
...
3. **state-shape = FSM có precondition** — cả 4 nguồn: transition là API gated, không phải gán trực tiếp (đã phủ ở `routing`). **@55cf3a4 bee đẩy dạng mạnh nhất:** không chỉ "transition gated" mà "state là sản phẩm của producer" — `compounding` chỉ tồn tại nếu `scribing-run` thật đã chạy, không ai gán được value. Chống đúng lỗ mà precondition-gating thuần bỏ sót: gán thẳng terminal-state để giả một quy trình chưa chạy.
...
3. **Tầng durability — mượn "5 mức bền" của symphony NGAY khi có branch/PR flow.** Phát biểu tường minh "chạy xong ≠ đã merge ≠ đã bền" tránh bug kiểu coi run thành công là đã-xong. forgent chưa có fleet-run nên chưa cấp bách, nhưng nguyên tắc rẻ để ghi vào doc từ đầu. **Cập nhật @05a131f:** bee tự chứng minh nguyên tắc này portable xuống một cơ chế cụ thể, không chỉ triết lý của symphony — `worktree-merge-staged-verify-gate` (git merge dàn trận, verify trước commit, main byte-untouched khi đỏ) là instance THỨ HAI, độc lập, của "đã chạy xong ≠ đã bền" ngoài symphony. Với forgent: khi có merge-back từ nhánh song song (worktree hay branch), mẫu **stage → verify trên cây chưa commit → commit chỉ khi xanh, else abort có bằng chứng** là bản rẻ, cụ thể hơn để bắt đầu — không cần dựng đủ 5-mức của symphony ngay.
...
5. **Tầng concurrency-safety — theo khuyến nghị deep-dive `routing`:** default single-writer kiểu bee + cài sẵn CAS rẻ (`expected-status` hoặc rename-lock) cho mọi ghi vào state dùng chung, phòng background job tương lai. **Cập nhật @55cf3a4:** bee tự đi hết một bước — từ single-writer sang đa-phiên-cùng-checkout bằng atomic claim (O_EXCL+TTL/heartbeat, reclaim đòi cả-hai) + lane + hold deny (`same-checkout-multi-session-coordination`, đã log candidate). Đây là reference-impl thật cho "state layer chịu multi-writer" — khi forgent lên multi-agent (đã chốt), đây là mẫu để theo, không phải tự nghĩ. Không lặp lại phân tích, xem `routing` §synthesis.
...
| same-checkout-multi-session-coordination | beegog:cross-session-atomic-claims + beegog:lane-scoped-pipelines | R3 E2 F3 | **MỚI @55cf3a4** (đã log porting-log) — state-shape đa-pipeline (lane) + concurrency đa-phiên (atomic claim/hold). Chứng minh bee tự đi con đường "state layer chịu multi-writer" mà điểm (5) tính tới; nền cho fan-out. E2: bee dogfood |
| chain-integrity-tail-guard | beegog:chain-integrity-guard-tail | R2 E2 F2 | **MỚI @55cf3a4** (đã log porting-log) — state-shape dạng mạnh: terminal-state là sản phẩm của producer thật, không set-value; chống giả-đóng-quy-trình. Nối AGENTS.md critical-rule 12 |
| staged-merge-durability-gate | beegog:worktree-merge-staged-verify-gate | R2 E3 F2 | **MỚI @05a131f** — instance thứ hai (độc lập symphony) của "chạy xong ≠ bền": stage merge → verify cây chưa commit → commit chỉ khi xanh, abort có bằng chứng khi đỏ. E3: hai nguồn độc lập hội tụ về cùng nguyên tắc, một nguồn có impl cụ thể cho git-merge. Rẻ hơn dựng đủ 5-mức durability của symphony để bắt đầu |
| named-store-lock-primitive | beegog:store-lock-named-mutex | R2 E2 F1 | **MỚI @05a131f** — một khoá đặt-tên chéo tiến trình (lockfile O_EXCL, staleness 2 tầng) dưới MỌI mutator store; nền cho bất kỳ multi-writer nào forgent cần, rẻ hơn tự nghĩ locking scheme riêng cho từng loại record |
| hot-cold-archive-transaction | beegog:cell-archiving-transaction | R1 E2 F2 | **MỚI @05a131f, CÓ ĐIỀU KIỆN** — tách hot-scan/archive có giao dịch-đảo-được cho JSON store khi số record tăng; chỉ đáng khi forgent gặp đúng bài toán scale (nhiều trăm feature/cell) mà bee đã gặp. R1: chưa cấp bách ở quy mô hiện tại |
