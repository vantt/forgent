...
- **Capture-recovery (MỚI @05a131f):** `crash-transcript-recovery-mining` — kênh capture THỨ BA, vá đúng lỗ hai kênh trên bỏ sót: một phiên CRASH (không đóng sạch, không hand-edit phase giả — nên `chain-integrity-guard-tail` không có gì để chặn) vẫn có thể mất settlement chưa kịp flush. Phát hiện rẻ (mỗi `bee status --json`: transcript thiếu "clean-end trio" + còn dấu việc dở); khai quật là ĐỀ NGHỊ người duyệt, không tự chạy; digest ≤600 từ nạp `capture-queue.jsonl` qua `capture add --source mined` — **chưa xác nhận**, đi qua đúng cổng người giống mọi settlement khác, không bao giờ tắt qua thành decision. Kỷ luật context đi kèm: CLI chỉ PHÁT prompt (không gọi LLM) nên transcript thô không chảy vào context orchestrator.
