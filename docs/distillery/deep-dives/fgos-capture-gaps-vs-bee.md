...
date: 2026-07-29
based_on: [bee@1.18.3]
entries: [bee:context-md-source-of-truth, bee:unattended-triage-front-door, bee:single-writer-context-md-render, bee:phased-chain-with-four-gates, bee:structured-decision-recall-surface]
---

# Deep-dive: fgOS's 4 capture gaps vs bee's state design

**Bottom Line:** bee's live `decisions.jsonl` (701 real entries, checked
...
fgOS's own capture mechanism was explicitly modeled on bee's ("cơ chế này
thực chất học từ họ"). tsk-ma4's audit found 4 gaps in fgOS's capture
design. Does bee's actual state design already solve these — meaning fgOS
regressed from a pattern bee already had, rather than never having built it?

## Cách bee giải quyết

### Gap 1 (fgOS: `gates[id]` has no actor/role) — bee: `source` field, live and rich
...
**Why bee designed it this way:** `source` defaults to `'user'` but every
...
### Gap 4 (fgOS: no rejected-alternatives capture anywhere) — bee: `alternatives` field, populated 64/701 times
...
(`decisions.mjs:307-308` throws if blank) — bee never allows a decision to
...
alternative to fgOS's RUL32 latest-wins `reason` field: bee keeps full
...
bee had both patterns and picked append-never-overwrite specifically for
`decisions.jsonl`.

### Gap 3 (fgOS: CONTEXT.md unenforced) — bee: mandatory Gate 1 + unattended equivalent
...
bee that advances a phase and writes nothing, which is exactly fgOS's
...
| | fgOS today | bee | Gap closed by copying? |
|---|---|---|---|
...
Concrete fgOS schema change, combining bee's proven shape with fgOS's own
append-only-log discipline (RUL11 — bee's shape fits this without
modification, since bee's `decisions.jsonl` is itself append-only):

1. **Extend `addDecision`'s payload** (`src/state/store.mjs:603`) to accept
   `rationale` (required, mirroring bee's throw-if-blank rule),
   ...
   distinction). Reuses bee's exact field names and semantics; no new
   concept to invent. This alone gives fgOS's `decision` verb structural
   parity with bee's, and is a natural superset of D4's `role`/`actor`
...
3. **Do NOT copy bee's `type: supersede` mechanism wholesale** — fgOS's
   `decision` verb is genuinely global/unscoped (confirmed in tsk-ma4's
   audit) while bee's is per-repo-state; adopting supersede meaningfully
...
4. **Gate 3's structural fix (bee's answer to gap 3) needs a phase-graph
...
- Bee's `source` free-text convention (`"bee-planning (bypass-total
  ...
  free text (bee's choice) or a closed enum (`human`/`session`/`engine`)?
