...
> Backfilled 2026-07-18 for the two domains this host's taxonomy adds beyond the
> original scan (`routing`, `integration-contract`). These sit one layer below the
> doc-level entries above — raw implementation, verified at HEAD `a0678a3`.

### hook-vs-screen-timestamp-arbitration
- **What:** The actual pane-status arbitration logic (not just the doc-level "authority table"): `effective_agent_label()`/`effective_known_agent()` prefer `hook_authority` over `detected_agent` (screen manifest), but `recompute_effective_state()` can still let a screen-detected visible blocker override a non-full-lifecycle hook's reported state via `visible_blocker_overrides_hook()`, gated by a timestamp freshness check (`fallback_not_older_than_hook()` / `hook_authority_not_newer_than()`) so a stale screen read can never clobber a fresher hook report.
- **Where:** `src/terminal/state.rs` — `recompute_effective_state` (L1352), `visible_blocker_overrides_hook` (L1277), `hook_authority_not_newer_than` (L511), `fallback_not_older_than_hook` (L517), `should_ignore_detected_state_under_full_lifecycle_hook` (L534)
- **Notable:** The override is conditional, not absolute: a visible blocker only overrides the hook when the hook is *not* full-lifecycle-authoritative, the hook's own state isn't already `Blocked`, the hook-reported agent matches the screen-detected agent, and the screen read isn't older than the hook's last report. A genuine tie-break rule with staleness protection, one layer below the "one authority per pane" summary already in this index.
- **Keywords:** effective_agent_label, hook_authority, visible_blocker_overrides_hook, timestamp staleness, tie-break rule
- **Seen:** a0678a3

### detection-rule-priority-and-reason-coding
- **What:** When multiple manifest rules match the same screen snapshot, `evaluate_loaded_manifest` picks the highest-`priority` match (first-seen wins exact ties), then encodes *why* into machine-readable strings: `skipped_update_reason = "matched_rule:<rule_id>"` when the winning rule sets `skip_state_update`, or a `fallback_reason` (`"unknown_agent"` for unrecognized agents, a `DEFAULT_KNOWN_AGENT_IDLE_FALLBACK` constant when a known agent has no matching rule) when nothing matches at all.
- **Where:** `src/detect/manifest.rs` — `evaluate_loaded_manifest` (L414), skip-reason assembly (L460), `fallback_explain` (L497)
- **Notable:** This is the concrete implementation behind `herdr agent explain`'s reason fields — the routing decision is required to justify itself with a string an operator or agent can read, not just return a bare enum state.
- **Keywords:** rule priority, skipped_update_reason, fallback_reason, matched_rule, agent explain
- **Seen:** a0678a3

### plugin-link-handler-routing
- **What:** `find_plugin_link_handler` decides which installed plugin's `[[link_handlers]]` regex owns a clicked URL: it filters to enabled plugins with an available manifest, sorts plugins deterministically by `plugin_id`, then walks each plugin's handlers in manifest declaration order, skipping any handler whose platform isn't supported or whose declared `action` doesn't exist in that plugin.
- **Where:** `src/app/api/plugins/mod.rs` — `find_plugin_link_handler` (L530), `invoke_plugin_link_handler_for_url` (L227)
- **Notable:** Skill/component-routing at the plugin layer — "which extension handles this event" — with an explicit, testable ordering contract (`plugin_link_handlers_keep_manifest_order_for_overlapping_patterns` test) rather than "first plugin found" being incidental.
- **Keywords:** link_handlers, plugin_id ordering, first-match routing, overlapping patterns
- **Seen:** a0678a3

## integration-contract

### wrapped-process-env-var-contract
- **What:** Herdr injects a fixed family of env vars (`HERDR_ENV=1`, `HERDR_SOCKET_PATH`, `HERDR_BIN_PATH`, `HERDR_WORKSPACE_ID`, `HERDR_TAB_ID`, `HERDR_PANE_ID`, plus `HERDR_PLUGIN_*` for plugin processes) into every pane/plugin process it launches; these Herdr-managed vars "stay authoritative" over caller-supplied `--env` values on conflict. Every one of the 14 official per-agent hook scripts gates itself on `test "${HERDR_ENV:-}" = 1` (or the language equivalent) before reporting anything back, and Herdr itself uses the same var to refuse to nest inside another Herdr session (`should_block_nested_for_env`) unless `experimental.allow_nested` is set.
- **Where:** `src/main.rs` — `HERDR_ENV_VAR`/`HERDR_ENV_VALUE` constants (L11–12), `should_block_nested_for_env` (L415–419); `src/pane.rs` (L115); `src/pty/backend/unix.rs` (L77); `src/app/api/plugins/panes.rs` (L248, L342); `src/app/api/plugins/runtime.rs` (L45); `src/integration/assets/` — the `herdr-agent-state` hook script (sh/ps1/ts/js/py), all 14 gate on it; `docs/next/website/src/content/docs/cli-reference.mdx` (L423–441)
- **Notable:** The concrete discovery/self-identification contract between Herdr and any wrapped coding-agent process: the wrapped process cannot address the local Herdr socket, cannot know its own pane/workspace identity, and (per the hook scripts) refuses to act at all unless this env-var handshake is present — distinct from `HERDR_AGENT` (a detection *hint*, captured under `sandbox-agent-hint`) and from `HERDR_ENV`'s appearance as a precondition test inside `shipped-agent-skill-file`.
- **Keywords:** HERDR_ENV, HERDR_SOCKET_PATH, HERDR_PLUGIN_*, env precedence, hook self-gating, nested-session guard
- **Seen:** a0678a3

### protocol-version-negotiation-check
- **What:** `check_client_version(client_version: u32) -> VersionCheck` is the runtime wire-compatibility check executed on every connection: version `0` (pre-persistence clients) is always rejected, an exact match is `Compatible`, an older client is rejected with an "upgrade your herdr client" message, and a newer client is rejected with an "upgrade the herdr server" message — the server never attempts partial/backward compatibility.
- **Where:** `src/protocol/wire.rs` — `VersionCheck` enum (L899), `check_client_version` (L914)
- **Notable:** Distinct from the `protocol-version-discipline` entry (governance rule for bumping `PROTOCOL_VERSION` relative to release tags): this is the runtime enforcement function, and its policy is strict two-way rejection rather than a compatibility range — no client is ever allowed to talk to a server on a different version in either direction.
- **Keywords:** VersionCheck, check_client_version, strict version rejection, Welcome error
- **Seen:** a0678a3

### external-agent-minimum-version-gate
- **What:** Before installing an official integration, Herdr can require a minimum version of the *other* product's own CLI: `agent_version_requirement()` declares a `min_version` per `IntegrationTarget` (currently Kimi, requiring `kimi --version` ≥ `KIMI_MIN_VERSION = "0.14.0"`), and `enforce_agent_version()` probes it, returning `Ok(None)` (satisfied), `Ok(Some(warning))` (version undeterminable — install proceeds with a warning), or `Err` (too old — install blocked).
- **Where:** `src/integration/version.rs` — `AgentVersionRequirement` (L9), `enforce_agent_version` (L42); `src/integration/mod.rs` — `KIMI_MIN_VERSION` (L64); `src/integration/actions.rs` (L121)
- **Notable:** The inverse of `min_herdr_version` (a plugin declaring the Herdr version it needs, covered under `plugin-trust-and-preview-gate`) — here Herdr imposes a minimum-version contract on an external agent product before wiring up the integration, with a three-way outcome (satisfied / unknown-so-proceed / too-old-so-block) rather than a hard boolean gate.
- **Keywords:** agent_version_requirement, enforce_agent_version, KIMI_MIN_VERSION, capability probe
- **Seen:** a0678a3
