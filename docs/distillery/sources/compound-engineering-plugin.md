...
- **Notable:** đây là bản triển khai NGHIÊM TÚC nhất của "one brain, N belts" trong toàn distillery — beegog:dual-runtime-contract chỉ projection sang 2 belt (.codex/.agents) bằng catalog-projection; CE dịch runtime sang 7+ target với tool/permission/hook/model map tường minh mỗi target. Parse một lần, convert N lần, write N lần; không dependency vào Claude runtime (chạy offline, thuần filesystem+JSON).
...
- **Notable:** "Bundle as serialization barrier" — converter sinh bundle không phải path đĩa, nên convert offline test được không I/O. Cùng gene policy-vs-ops với beegog:policy-vs-ops-split nhưng ở tầng build: quyết-định-dịch (converter) tách khỏi tác-động-đĩa (writer). Registry là plain data+function pairs, không polymorphism nặng.
...
- **Notable:** cùng bài "one truth, N projection, drift test" với beegog:hook-catalog-projection nhưng cho manifest đa-platform: khi bạn commit metadata cho ~11 nơi, parity check là thứ chống drift. Version "3.19.0" nhân bản qua mọi manifest do release automation quản; marketplace version tách riêng, tự do lag.
...
- **Notable:** đối cực với symphony:product-boundary-non-goals (một product, một runtime): đây là một artifact, N nhà phân phối. So với beegog dual-runtime (2 belt qua projection machinery), CE chọn commit-native mọi belt + converter cho phần còn lại — "native khi platform có surface, convert khi không". Chỉ đáng học khi forgent thật sự đa-platform; hôm nay là YAGNI nhưng là bản đồ đầy đủ nếu tới.
...
- **Notable:** dogfood-as-skill với report durable resume-được — cùng gene checkpoint-survives-compaction với disk-as-source-of-truth (ce-optimize) và bee HANDOFF. "Diff-scoped" là chìa: test cái branch này đổi, không phí sức cả app.
...
- **Notable:** một artifact tiến hóa theo readiness thay vì nhiều doc bàn giao — cùng bài dual-audience mà beegog:briefing-projection-artifact giải bằng projection (plan terse → brief human-readable). CE giải khác: một file, hai readiness level, enrich in-place. Đối chiếu cũng với repository-harness:spec-decomposition-lifecycle ("spec là input, không phải truth").
...
- **Notable:** context-seed cho cả fleet skill — product-pulse đọc STRATEGY.md để đo đúng metric user nói quan trọng. Cùng vai "một truth ngắn nuôi nhiều consumer" với beegog CONTEXT.md nhưng ở tầng product/strategy, không phải feature.
...
- **Notable:** đây là beegog go-mode / bypass-total ở dạng skill tuyến tính cứng thứ tự — "plan MUST complete before work" chính là bee critical-rule-1 (never execute before validating) đóng thành step gate. Khác bee: không human gate giữa chừng (autonomous), bù lại bằng evidence gate cơ học mỗi bước.
...
- **Notable:** headless của forgent (distill có, bee có) mới là một cờ; CE nâng thành CONTRACT ba-mode nhất quán toàn bộ skill — cùng skill hành xử khác nhau tùy audience (người vs orchestrator vs pipeline). Là điều kiện để skill compose được (lfg gọi ce-work ở return-to-caller mode).
...
- **Notable:** HỘI TỤ ĐỘC LẬP với bee critical-rule-13 ("fan out the gathering, keep the deciding") — CE tới cùng luật từ hướng PR-review: judge-altitude ở orchestrator, fan-out chỉ cho I/O/fix. "Judge centrally" bắt được reviewer sai hệ thống mà fan-out-judge bỏ lỡ. Tín hiệu mạnh cho routing-model-per-interface candidate.
...
- **Notable:** envelope-return là cách skill thành sub-routine composable — cùng gene "worker trả digest" với bee Delegation contract, nhưng đây là skill-gọi-skill (không phải model-gọi-subagent). Handoff menu = routing prose giữa skill trong chain, đối chiếu beegog:hive-first-skill-router.
...
- **Notable:** đúng "ceremony scales, memory never" của beegog:risk-lanes-mechanical, và fail-closed giống bee guard discipline (uncertainty → nhiều ceremony hơn, không ít hơn). Bản triển khai cụ thể một ngưỡng đếm-được (39 exec line) cho lite lane.
...
- **Notable:** THỨ KHÔNG NGUỒN NÀO KHÁC CÓ — eval/optimize loop có kỷ luật như một experiment framework. LLM-as-judge mode đặc biệt hợp khi forgent cần chấm chất lượng output agent (không có metric cứng). Disk-as-truth ở đây là bản mạnh nhất của beegog:state-vs-log-two-physics.
...
- **Notable:** catalog event RỘNG HƠN beegog:lifecycle-coverage (7 hook/7 event) — CE thêm PostToolUseFailure, PermissionRequest, SubagentStart/Stop. Action type `agent` (hook dispatch một subagent) là thứ bee hook chưa có. Quan trọng khi forgent map hook đa-runtime: đây là bề mặt event đầy đủ để chống drift.
...
- **Notable:** an-toàn-ghi cấp installer: giả định user CÓ THỂ đã fork/symlink file mình, và không bao giờ giẫm lên. Cùng tinh thần fail-safe với beegog:fail-open-crash-wrappers (crash không lật allow/deny) nhưng cho filesystem write. Cặp với install-manifest-ownership.
...
- **Notable:** default-on-không-hỏi là UX doctrine đối lập bee silent-bookkeeping nhưng cùng đích "đừng bắt người quyết cái máy tự làm được" — CE tự babysit trừ khi bảo đừng; bee chạy machinery im lặng. "Feedback before CI" là ordering invariant đáng học cho bất kỳ PR loop nào.
...
- **Notable:** bản áp-dụng-rộng của beegog:state-vs-log-two-physics + bee HANDOFF discipline — CE thực thi "disk là truth, context là ephemeral" ở MỌI skill dài, với write-then-read-back verify. "Results table in conversation without writing to disk first = a bug" là câu chốt đáng port thành doctrine.
...
- **Notable:** grounding chung được cache một lần dùng nhiều skill — tránh mỗi skill re-scout project từ đầu. Cùng bài "đừng re-read cái đã biết" với distill cursor/inventory-reuse và bee rule-13 (không re-read digest). Cache profile = memoized grounding.
