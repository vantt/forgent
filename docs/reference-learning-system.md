...
- `integration-contract` — hợp đồng versioned giữa HAI hệ/sản phẩm agent RIÊNG BIỆT: capability negotiation, discovery-before-mutation, exit-code/timeout/output semantics, product-boundary ownership ("owns X / KHÔNG owns Y", non-goals), forward-compat rules. Ranh giới: `orchestration` = điều phối worker TRONG một hệ; `routing` tầng 3 = *quyết định* chạy gì kế tiếp DỰA TRÊN hợp đồng (branch-on-exit-code, rediscover-on-timeout); `integration-contract` = *bản thân hợp đồng/ranh giới* mà các quyết định đó chạy trên. Không tính portability một-codebase-nhiều-runtime (đó là `harness`, vd bee:dual-runtime-contract).
...
- `status [--json]` — dashboard: cursor/behind từng nguồn, intake chờ, candidate chưa score, deep-dive stale (--json cho hook/cron sau này)
- `find <term>` — in entry blocks + matrix rows khớp term (thay công thức grep 3 bước thủ công)
