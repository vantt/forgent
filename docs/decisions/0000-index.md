...
| [0033](0033-multi-role-team-harness-role-holder-axis-va-handoff.md) | Multi-role Team Harness: trục role/holder, handoff | Work item thêm trục thứ ba `role/holder` (opt-in per-domain qua `roleGraph`), verb `handoff`/`handoff-return` guard theo edge hợp lệ; call 4 reason (advise/assist/review/consult) round-trip vs pass một chiều; ontology 4 tầng task-spec/skill/knowledge/context; hierarchy domain → N workflow qua selector `kind`; cockpit absorption là khách hàng đầu tiên. Distill 24 vòng thảo luận tsk-2t9c (D1–D18). Đánh số lại từ `0032` vì đụng số với record iron-law tạo trên nhánh song song (tsk-60r4). |
