...
| [0029](0029-sua-dinh-nghia-roottask-subtask-capacity-t1-cua-0026.md) | Sửa ba mệnh đề từ vựng dispatch của `0026` | Bỏ `rootTask`/`subTask` khỏi từ vựng (thay bằng `work`/child work); `capacity` = behavior-promise + functional-helper; T1 (vai trò bên gọi) chỉ có hai giá trị `launcher`/`driver`, tên gọi ban đầu của `0026` là tầng hợp thành T0. Supersede `0026`. **D8's cặp behavior-promise/functional-helper sau đó được TÁCH thật trên code (`tsk-34n`) rồi chốt tên chính thức bởi [0034](0034-doi-ten-capacity-thanh-executor-chot-tach-capability.md): behavior-promise = `capability` (giữ nguyên), functional-helper = `executor` (đổi tên từ `capacity`). Văn bản gốc D8 không sửa.** |
...
| [0033](0033-cli-spawn-shaped-capacity-thang-hasLiveTaskAccess.md) | Capacity cli-spawn-shaped thắng `hasLiveTaskAccess`, thu hẹp `0026` rule 2 | Một capacity `kind:agent` cli-spawn-shaped (có `command`/`adapter`/`invocations[].via==='cli'` riêng, ví dụ `agy`) giờ luôn dispatch out-of-process khi đã cấu hình, bất kể caller có Task tool sống hay không — quyết định người dùng trực tiếp 2026-08-16. Capacity agentType-shaped (chỉ `agentType`, không command riêng, ví dụ `judge-discovery`) giữ nguyên hành vi cũ, vì honoring nó in-process đã là honoring cấu hình. Extends `0026`, thu hẹp riêng rule 2 cho nửa case cli-spawn-shaped; 0 test hiện có bị gãy (xác nhận qua quét toàn bộ `test/runner/dispatch.test.mjs`). **Chữ "capacity" trong văn bản gốc record này KHÔNG đổi (lịch sử) — [0034](0034-doi-ten-capacity-thanh-executor-chot-tach-capability.md) đổi tên khái niệm này thành `executor` cho code/config đi tới.** |
| [0034](0034-doi-ten-capacity-thanh-executor-chot-tach-capability.md) | Đổi tên `capacity`/`capacities` thành `executor`/`executors`, chốt tách `capability` | `runner.capacities.<id>` đổi tên dứt điểm thành `runner.executors.<id>` — field config lẫn mọi identifier code, không back-compat; `capability`/`executor` (số ít, default toàn cục) giữ nguyên. `executor` thắng phương án `backend`: kỹ thuật (khớp shape `resolveExecutorConfig` đã dùng sẵn) + ngữ nghĩa (capability = lời hứa, executor = hiện thực hóa lời hứa). Chính thức hóa việc `0029` D8 gộp behavior-promise/functional-helper làm một, tsk-34n sau đó tách ra mà chưa từng sửa D8. |
| [0035](0035-xac-lap-ranh-gioi-su-menh-fgos.md) | Xác lập ranh giới sứ mệnh fgOS: phục vụ project khác/business workflow, không tự-phát-triển mặc định | fgOS tồn tại để (1) phát triển project khác và (2) vận hành business workflow — tự-phát-triển chính nó (mission #3) là dogfood, không phải sứ mệnh. Ranh giới self-vs-host là một trục quyết định riêng, đứng CẠNH (không nối vào làm bậc #5) thang ưu tiên `0030`. Cơ chế nhận diện: config key `mission` (`self-dev`\|`host`) khai báo một lần lúc setup, không hỏi per-decision, không mechanize repo-divorce kiểu bee. `tsk-1js` (Iron Law `MODULE_RULES` hard-code path của chính fgOS) là ứng viên thi công đầu tiên, không phải dependency. |
