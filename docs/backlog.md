...
| P11 | Ly khai bee (xưởng, gốc) ↔ forgent (sản phẩm, ./repo): artifacts hai bên tách tuyệt đối, release sạch không rác | Baseline verify xanh nguyên trạng trong ./repo; một cell bee smoke chạy trọn vòng ghi vào ./repo; fgos ready + runner --dry-run chạy đúng chỗ mới; doctrine hai bên tách đôi | in-flight | repo-divorce |
| P12 | Export chưng cất sử ký sản phẩm: sinh decision-records/changelog vào docs/decisions/ của forgent từ hồ sơ xưởng (per D3 repo-divorce) | Người ngoài đọc docs/decisions/ hiểu các quyết định lớn của sản phẩm mà không cần vào xưởng | proposed | — |
