...
| `dispatch/confinement/` | Confinement Authority subcomponent: one-door runtime execution wrapper (`executeThroughConfinement`), request builder, plan compiler, policy normalizer, backend registry and drivers (local `bwrap` v1 with 8 falsification probes), attestation generator, and evidence store. See `docs/specs/confinement-authority.md`. |
