...
- `retrospective`'s own staleness threshold (D4) — not chosen here, left to
  `tsk-1bl`'s implementer.
- Poll cadence for `tsk-2xt`'s herdr-side detection loop, and whether herdr
  ...
  session, left to `tsk-2xt`'s implementer.
