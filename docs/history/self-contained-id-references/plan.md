...
| Wiring a real-repo-scan into `npm test` (not just unit tests against synthetic fixtures) | medium — this is the one piece the existing script family has never done (confirmed: neither `check-decision-citation-drift.mjs` nor `check-decision-supersession.mjs` has a real-repo `npm test` case; `check-decision-codes.mjs` DOES, and is the pattern to copy) | one new test case in `test/scripts/check-decision-citation-drift.test.mjs` that spawns the CLI with `cwd` = repo root against the checked-in baseline, asserting exit 0 |
