...
| `test/e2e/rebuild-determinism.test.mjs` | Update the `run(cwd, ['decision', '--text', 'locked D3: ...'])` call (line 94) to also pass `--rationale` — found by a full-repo grep at `fgos-validating` time, missed by the original single-caller check (that check only covered `addDecision`'s direct code caller, not every test invoking the CLI command). | D2 |
...
| `test/state/replay.test.mjs` | Add: a decision event carrying `id` folds into `view.decisionsById[id]` as an accumulating array while the global `view.decisions` array keeps receiving every decision (id-bearing or not) exactly as today. | D1, seq 1190 |
