...
| `package.json` | low | Item's footprint lists it defensively; no change identified as necessary yet beyond what already exists (`scripts.test` already runs the right glob). Pin as assumption — if `fgos-executing` finds a real need (e.g. a lockfile, a `ci` script alias), it's a small addition, not a redesign. |
...
- No new verify command needed for piece 3 (the workflow file itself) —
  ...
  `fgos-validating`/`fgos-executing` can check via `gh run list`/`gh run
  view` once pushed.
