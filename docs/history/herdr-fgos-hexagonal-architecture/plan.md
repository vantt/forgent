...
| `src/app.rs` | D2: `App.selected` becomes a plain `Option<usize>`; `select_next`/`select_previous`/`selected_id`/`clamp_selection` operate on it directly. Drop the `ratatui::widgets::ListState` import — after this change `app.rs` imports nothing from `ratatui` or `std::process`. |
...
| `src/main.rs` | Becomes the composition root: construct `FgosCliSource`, `HerdrPaneAdapter`, and the ratatui `TerminalUi` adapter; `run()`'s loop calls only `TerminalUi::poll_event`/`draw`, `WorkItemSource` methods, and `PaneOrchestrator::open_pick_pane` — no direct `crossterm`/`ratatui`/`std::process::Command` call left in this file. |
