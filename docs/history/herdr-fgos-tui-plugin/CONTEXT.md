...
| D4 | "In-process task" list = fgOS items with `status: doing`, read via the fgOS CLI (`fgos list --json` / `fgos triage --json`) — **never** herdr's own `agent_status`. This is not a new call: it is the same hard rule STR40 already locked (decision `d3dbe7f5`; superseded 2026-09-06 by "RUL40 re-scope" — herdr still never truth, but agent API allowed as transport/failure detector) after a real production bug where herdr's own agent-detection became a second, competing source of truth ("idle killed an agent"). This plugin inherits that rule, does not re-decide it. |
