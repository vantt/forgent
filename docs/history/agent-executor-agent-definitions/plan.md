...
Decisions: `docs/history/agent-executor-agent-definitions/CONTEXT.md` (D1, D2, D3).
...
2. Write one small projection script,
...
No existing file is edited. `fgos graph --what-if tsk-slq` shows this item
unblocks only `tsk-64p` (the milestone) — it is not on the tracked critical
path (`fgos graph`'s `criticalPath` does not include `tsk-slq` or `tsk-62v`)
and has no declared dependency on or from `tsk-62v`, its sibling under
`tsk-64p` — the two can build in either order.
...
- YAML parsing library — repo's existing dependency set decides; if none
  present, hand-rolled minimal parse is acceptable given the schema's
  intentionally small, flat-ish shape (matches `.fgos-runner.json`'s own
  precedent of avoiding a new dependency for a small config shape).
