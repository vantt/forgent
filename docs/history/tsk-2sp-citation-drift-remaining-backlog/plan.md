...
| Cross-child footprint on `scripts/check-decision-citation-drift.baseline.json` | Real, intentional — all five children regenerate this one shared file, so `footprintOverlapAmong` (`src/state/graph-metrics.mjs:599-623`) will report all five as pairwise-conflicting on this one path. This is honest, not a defect: the same JSON array gets rewritten by whichever child runs `--write-baseline` last, so parallel dispatch of two of these children risks one clobbering the other's regen. Verified this is the ONLY shared path (every other footprint entry is unique to its own child — see "Assumptions" row in the Gate section) | Serialize these five children rather than fan them out concurrently; each child's own verify re-runs `--write-baseline` fresh against its own file's real state regardless of dispatch order, so serial execution is correct and sufficient, no extra proof needed beyond each child's own verify passing |
