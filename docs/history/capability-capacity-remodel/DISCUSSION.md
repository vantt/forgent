...
| 3 | Có nên giữ khả năng override bằng key literal không (một item tự đặt `capacities.<đúng-tên-purpose>` riêng để ép dùng backend khác)? | ĐANG HỎI — có phân tích | Đọc `src/runner/dispatch.mjs:1886-1890` (`decideCapacityCli`'s `--work` branch): chỗ cần thêm fallback CHÍNH LÀ đúng chỗ `hasExplicitCapacity` đã kiểm tra key literal trước rồi mới rơi về native-default. Thêm `for`-fallback vào ĐÚNG SAU bước literal-key đó (chưa tìm thấy literal key → thử `resolveCapacityIdForPurpose` → vẫn không thấy → mới rơi về native-default cũ) nghĩa là override-bằng-key-literal không phải một tính năng cần THÊM — nó là thứ tự tự nhiên, miễn phí, giống hệt pattern `capacities.<id>` > `executor` global đã có sẵn ở `resolveExecutorConfig`. Bỏ nó đi mới là việc tốn công (phải chủ động cấm literal key thắng). Khuyến nghị: giữ, không phải chọn giữa hai thứ — literal luôn thắng trước (không đổi hành vi cũ), `for` chỉ là fallback mới khi không có literal. |
