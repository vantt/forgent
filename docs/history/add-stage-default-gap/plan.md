...
real parallel workability (per fgos-planning's own "smallest honest
plan" framing).
...
- `.claude/skills/fgos-planning/SKILL.md` step 4 and its mirrored
  `.agents/skills/fgos-planning/SKILL.md` copy: update the split-child
  ...
  to `decompose` for `fgos-validating`'s reality check, not repeat a full
...
checked 2026-08-06T08:27Z) — worth a `fgos-validating` blast-radius read
on `bin/fgos.mjs`'s `add` case specifically, since it is called by every
`fgos-planning` split today (D1's own stated scope).
...
it at `decompose` lets `fgos-planning`/`fgos-validating` review that
...
- The replacement item's own execution (once it reaches `executing`)
  still needs `fgos-validating`'s real reality check — this plan only
