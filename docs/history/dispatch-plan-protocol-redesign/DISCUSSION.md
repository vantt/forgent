...
| 11 | Herdr-as-transport (pha 6/7 mới) có consumer thật đang chờ không, hay vẫn là tầm nhìn chưa ai cần (cùng dạng YAGNI `tsk-6db` đã tự khoá)? | **CHƯA RÕ — câu hỏi mới, cần người quyết** | Scout: Herdr ĐÃ có tích hợp thật trong repo hôm nay — `fgos gateway start\|stop\|status` (`tsk-31v`, đã ship) chạy REST API + web dashboard trên Herdr; `.fgos/config.json`'s `herdrOrchestrator` (`autoDiscover/autoMerge/autoRetro/autoCleanup`) đã điều khiển một số automation loop; gateway mount cả MCP surface (`search`/`execute`, `herdr-plugin/src/mcp.rs`) trên cùng process. NHƯNG đây là bề mặt REST/dashboard/automation-trigger, KHÁC với "giao AgentMessage qua Herdr-managed runtime/session" mà pha 6/7 của người dùng mô tả (`HerdrTransport.deliver(message, plan)` — spawn/wake/route input vào terminal, chờ/stream response). Chưa thấy bằng chứng có ai cần đúng bề mặt thứ hai này hôm nay. |
