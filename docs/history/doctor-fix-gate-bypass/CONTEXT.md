...
- `docs/history/global-project-config-awareness/CONTEXT.md` (nhánh
  `fgw/tsk-2ta`) — chủ thật của shared config file D1 chờ.
- `tsk-1qm` (`deps: [tsk-2cs, tsk-2qz]`) — nơi supersede RUL9/RUL11 chính
  thức trong spec, không phải item này.

## Outstanding questions deferred to planning

- Chính xác `fgos-planning` có wire `deps: [tsk-2ta]` chính thức vào graph
  hay chỉ giữ như một proof point trong plan.md (giống tsk-2cs đã làm) —
  quyết định dựa trên trạng thái merge thật của `fgw/tsk-2ta` tại thời điểm
  planning, không giả định trước ở đây (D1).
