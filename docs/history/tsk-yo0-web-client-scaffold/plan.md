...
|---|---|---|
| Vite/TypeScript/Tailwind scaffold builds cleanly | Low — following documented, current install paths for well-known, actively maintained tooling | `npm run build` + `npm run typecheck` both exit 0 (the item's own verify) |
| API client types match the contract yaml | Medium — a hand-drift between the client's TypeScript types and the contract's schemas would silently produce a client that compiles but lies about response shape | A unit test (vitest, since D14 locks it alongside vite/TypeScript) per contract operation asserting the client parses a fixture response shaped exactly like the contract's own example/schema — not just that the function compiles |
| Gateway-unreachable is actually distinguishable from an `ErrorEnvelope` | Low-Medium — an easy place to accidentally collapse both into one generic "error" the UI can't tell apart, which would violate the area spec's own locked edge case | A unit test mocking a network-level fetch rejection (not an HTTP error response) and asserting the client surfaces a distinct error kind/tag from an `ErrorEnvelope`-shaped 4xx/5xx response |
