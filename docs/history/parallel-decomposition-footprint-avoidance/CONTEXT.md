...
|---|---|---|
| D1 | Children = `computed-parallel-wave-schedule`-shaped work + `worktree-dispatch-attestation`-shaped work (2 candidates from `docs/distillery/porting-log.md`, deep-dive `docs/distillery/deep-dives/parallel-decomposition-and-merge.md`) | User directed using this pre-existing item as root instead of submitting duplicates |
