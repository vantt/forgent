...
- **`retrospective`** — the (former) `compound-learn` synthesis work
  (settlement/decision/learning/enduser-docs via `fgos-compounding`),
...
| D9 | `retrospective` is processed by a **separate loop**, run once per invocation, scanning every item currently at `delivered` — never inline in `return`/`approve`. `return`/`approve` now stop exactly at `delivered`. This is where the (former) `compound-learn` stage's synthesis work (settlement/decision/enduser-docs via `fgos-compounding`) relocates. Batch-trigger threshold (N items / T time) for this loop is explicitly **not** decided here — deferred, matches the still-open item (2) on `tsk-4op`/`tsk-3o3`. |
...
| D11 | Stage `compound-learn` is retired outright — **supersedes RUL49/RUL50/RUL51** (not just RUL50's gate-check; the stage itself, its edge `executing->compound-learn`, and the `compound` verb). The `fgos-compounding` skill now triggers on status `retrospective` instead of stage `compound-learn`. Keeping both would be two mechanisms doing the same reflect/learning job on two different axes — accepted as redundant technical debt if not retired now. |
