...
| D4 | (tsk-1hb, added during Execute after D2 alone was found to regress `parse_doing_excludes_done_items` — concrete evidence: implementing D2's predicate verbatim and running `cargo test parse_doing` fails that test, because `parkReason` is *also* absent for `todo`/`done`/`wontfix`/`delivered`/`retrospective`/`cleanup`, not just `doing` — `parkReasonForStatus`'s table only declares `blocked`/`awaiting-human`/`awaiting-approval`, `src/state/workflow-stage-graphs.mjs:192-196`.) `parse_doing`'s membership predicate is `parkReason` (D2's absent-or-`natural-finish` test) **AND** `statusCategory` is `"in-progress"` or `"review"` (`DOMAINS.coding.statusLabels`, `src/state/workflow-stage-graphs.mjs:159-166`) — both already-public, write-time-stamped fields (decision record 0027 for `statusCategory`, D1 above for `parkReason`), neither a raw literal `status` re-check, so this stays within D3's "no literal status match" intent while restoring the todo/done/wontfix/tail-segment exclusion the original literal check gave for free. Verified case by case: `todo`→category `"todo"` excluded; `wontfix`→`"canceled"` excluded; `done`/`delivered`/`retrospective`/`cleanup`→no `statusLabels` entry (`undefined`) excluded; `doing`→`"in-progress"`+`parkReason` absent → included; `blocked`→`"in-progress"`+`"system-error"` excluded; `awaiting-human`→`"in-progress"`+`"human-question"` excluded; `awaiting-approval`→`"review"`+`"natural-finish"` included. Reproduces the pre-this-item membership byte-for-byte. |
