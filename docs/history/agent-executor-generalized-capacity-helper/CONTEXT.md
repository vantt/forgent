...
- Exact file path/name for the shared fragment (D2) — implementation
  shaping, `fgos-planning`'s call once `tsk-3sw`'s shape is real to design
  against.
- Whether `tsk-53h` itself should decompose into two children (a
  ...
  `fgos-planning`'s shaping call, not decided here per this skill's own
