...
Add one new hard rule to each of `fgos-planning/SKILL.md`,
`fgos-validating/SKILL.md`, `fgos-code-implement/SKILL.md` (both
`.claude/skills/` and `.agents/skills/` — kept byte-identical, same
discipline `fgos-exploring`'s own fix already followed), forbidding ad hoc
...
committed to `fgos-exploring/SKILL.md` (`2bc193d`, corrected `8c1dab1`),
never copy-pasted verbatim: each rule names *that skill's own* reasoning
surface, not `fgos-exploring`'s.

**Alternative rejected:** rely on `fgos-validating`'s existing D6
...
alone would leave `fgos-validating` inconsistent with the other two files.
...
- `.claude/skills/fgos-validating/SKILL.md` + `.agents/skills/fgos-validating/SKILL.md`
- `.claude/skills/fgos-code-implement/SKILL.md` + `.agents/skills/fgos-code-implement/SKILL.md`
...
|---|---|---|
| Rule text drifts from what `CONTEXT.md`/this plan describes | low | `fgos-validating`'s reality gate reads the actual diff against this plan |
...
prior commit — degraded, not full — noted here only because `fgos-exploring`'s
...
1. In each skill's `## Hard rules` list, add a bullet forbidding ad hoc
   ...
   - `fgos-planning` — step 3 "Approach" (risk map, ordering) and step 4
     "Shape" (the plan itself) are the reasoning this skill exists to do
     directly.
   - `fgos-code-implement` — step 2 "Implement" (and, narrower, step 4's
     Iron Law classification) is the reasoning surface.
   - `fgos-validating` — step 2 "Reality gate" and step 3 "Feasibility
   ...
   `fgos-exploring`'s own rule, reworded per skill.
...
land in a single commit, same as `fgos-exploring`'s own fix did.

## Proof surface

```bash
for f in .claude/skills/fgos-planning/SKILL.md .claude/skills/fgos-validating/SKILL.md .claude/skills/fgos-code-implement/SKILL.md .agents/skills/fgos-planning/SKILL.md .agents/skills/fgos-validating/SKILL.md .agents/skills/fgos-code-implement/SKILL.md; do grep -q "capacity-dispatch-fallback.md" "$f" || exit 1; tr "\n" " " < "$f" | grep -qi "never delegate" || exit 1; done && node --test test/skills/fgos-mirror.test.mjs
```
...
second-pass dispute during `fgos-exploring` — the earlier drafts accepted
...
- Each new rule's exact wording is an implementation-only detail
  (`fgos-code-implement`'s call, matching the pattern already proven on
  `fgos-exploring`) — not material to `CONTEXT.md`'s locked decisions, so
  no mid-planning gap back to `fgos-exploring` is needed (step 7).
