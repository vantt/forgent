...
product decision to lock (no `fgos-exploring` pass ran, so there is no
D-ID to cite).
...
1. `fgos-validating`'s `## Handoff` section (currently ends by pointing at
   `fgos-routing` once the item reaches `executing` — see
   `.claude/skills/fgos-validating/SKILL.md:213-220`) — state plainly that
   the `decompose`→`executing` edge (`releaseClaimOnExecuting`,
   `src/intake/decompose.mjs:488-494`) drops the item's claim back to
   `todo`, so any path that continues by hand-calling `fgos-code-implement`
...
2. `fgos-code-implement`'s Orient step (currently reads title/refs/deps/
   docsRef only — `.claude/skills/fgos-code-implement/SKILL.md:73-78`) —
...
- `.claude/skills/fgos-validating/SKILL.md` — `## Handoff` section, one
  new sentence.
- `.agents/skills/fgos-validating/SKILL.md` — same sentence, mirrored copy.
- `.claude/skills/fgos-code-implement/SKILL.md` — Orient step (flow item
  1), one new sentence.
- `.agents/skills/fgos-code-implement/SKILL.md` — same sentence, mirrored
  copy.
...
```
npm test && [ "$(grep -c "claim may already be released" .claude/skills/fgos-validating/SKILL.md)" = "1" ] && [ "$(grep -c "claim may already be released" .agents/skills/fgos-validating/SKILL.md)" = "1" ] && [ "$(grep -c "claim may already be released" .claude/skills/fgos-code-implement/SKILL.md)" = "1" ] && [ "$(grep -c "claim may already be released" .agents/skills/fgos-code-implement/SKILL.md)" = "1" ]
```
