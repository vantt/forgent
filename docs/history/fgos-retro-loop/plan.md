...
## Revision (post-`fgos-validating` reality-gate FAIL)

The original `small`-mode plan (below, kept for the record) assumed
`fgos-compounding`'s step 3 (`fgos compound <id> --doc-type <quadrant>
--doc-path <path>`) already worked and only its *trigger-description
prose* was stale. `fgos-validating`'s reality gate disproved this
concretely:
...
- So `fgos-compounding/SKILL.md` step 3, as written today, calls a verb
...
3. `.claude/skills/fgos-compounding/SKILL.md` — fix the stale trigger
...
prose-only doc correction (`fgos-compounding/SKILL.md`) — no `impact()`
needed for those.
...
5. `fgos-compounding/SKILL.md` doc fix (depends on 1 existing — the
...
split" test (`fgos-planning`'s step 5) is whether more than one candidate
