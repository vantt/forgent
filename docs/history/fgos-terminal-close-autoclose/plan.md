...
|---|---|---|
| `--autoClose` token parsing added to `pick`/`discover`'s existing `$ARGUMENTS` read | medium — a plain `/fgOS:pick <id>` call (no flag) must stay byte-identical in behavior | `npm test` (existing suite covers today's plain-argument shape) + manual trace: parsing only strips a trailing `--autoClose` token, never changes id extraction when absent |
