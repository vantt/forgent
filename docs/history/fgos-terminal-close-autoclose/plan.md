...
|---|---|---|
| `--autoClose` token parsing added to `pick`/`discover`'s existing `$ARGUMENTS` read | medium — a plain `/fgOS:pick <id>` call (no flag) must stay byte-identical in behavior | `npm test` covers the underlying `fgos pick`/`fgos take` CLI verbs only (unchanged by this item) — it does NOT and cannot assert `$ARGUMENTS`-parsing prose behavior (`docs/how-to/write-verify-for-a-skill-prose-change.md`'s own scoping: no shell command asserts SKILL.md runtime behavior). Real proof surface for this row is `fgos-validating`'s own read-through at implementation time (parsing only strips a trailing `--autoClose` token, id extraction unchanged when absent) plus that doc's documented post-merge channel (`.fgos/events.jsonl` observed on real dispatch) — not a pre-merge shell assertion |
