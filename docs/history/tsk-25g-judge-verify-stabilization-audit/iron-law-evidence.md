...
```
top=$(git rev-parse --show-toplevel); root=$(git rev-parse --path-format=absolute --git-common-dir | xargs dirname); grep -A3 "secondPass: judgeVerifySemanticCorrectness(" "$top/src/intake/decompose.mjs" | grep -q "priorRejection" && grep -A25 "if (disputedChild)" "$top/src/intake/decompose.mjs" | grep -qE "\.force\b|force ===" && node "$root/bin/fgos.mjs" list --id tsk-25g --json --dir "$root" | grep -q "D1-resolved:"
```
...
```
$ grep -A3 "secondPass: judgeVerifySemanticCorrectness(" src/intake/decompose.mjs | grep -q "priorRejection"
NOMATCH

$ grep -A25 "if (disputedChild)" src/intake/decompose.mjs | grep -qE "\.force\b|force ==="
NOMATCH
...
```
$ grep -A3 "secondPass: judgeVerifySemanticCorrectness(" src/intake/decompose.mjs | grep -q "priorRejection" && echo "check1 GREEN" || echo "check1 RED"
check1 GREEN

$ grep -A25 "if (disputedChild)" src/intake/decompose.mjs | grep -qE "\.force\b|force ===" && echo "check2 GREEN" || echo "check2 RED"
check2 GREEN
...
with `outcome: "clear"` at the `fgos-planning` gate, no `--force` used
...
tests in `test/intake/decompose.test.mjs`, `test/state/replay.test.mjs`,
