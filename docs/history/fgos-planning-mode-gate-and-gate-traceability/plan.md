...
| existing covered behavior | **Có** | Dời mode-gate đổi LUỒNG HIỆN CÓ mọi phiên `fgos-planning` từng chạy qua — mọi session tương lai đi qua đường mới (routing quyết trước, không phải planning quyết sau khi đã nạp) |
...
liên quan chặt (cùng chủ đề tái cấu trúc `fgos-planning`), touch chung 2
...
|---|---|---|
| D1: dời mode-gate sang `fgos-routing` | Trung bình — đổi luồng hiện có, phải giữ NGUYÊN nội dung logic (mechanical flag-count), chỉ đổi VỊ TRÍ | Đọc lại `fgos-planning/SKILL.md`'s bước 2 gốc trước khi xoá, xác nhận nội dung mới trong `fgos-routing` giữ đúng 100% logic đếm cờ (không diễn giải lại) |
...
1. Xoá bước 2 ("Mode gate (mechanical, not vibes)") khỏi `fgos-planning/
...
3. `fgos-planning`'s Bootstrap (bước 1 cũ) đọc mode đã quyết từ
...
1. Thêm 1 rule vào Gate step của `fgos-planning/SKILL.md` (cả 2
...
`! grep -q 'Mode gate (mechanical' .claude/skills/fgos-planning/SKILL.md && grep -qE 'lane|mode.gate' .claude/skills/fgos-routing/SKILL.md && grep -qE 'truy.nguồn|trace back to|Open Question' .claude/skills/fgos-planning/SKILL.md`

## Assumptions

- Cơ chế truyền mode đã quyết từ `fgos-routing` sang `fgos-planning`
  ...
  truyền).
- Đánh số lại các bước còn lại trong `fgos-planning/SKILL.md` sau khi
