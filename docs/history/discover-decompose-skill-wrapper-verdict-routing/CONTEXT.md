...
| D6 | (fgos-planning gap, confirmed material, human-answered) Every invocation of `fgos-coding-driving` this item introduces carries an explicit ceiling — `discover/SKILL.md`: `ceiling: stage:decompose`; `decompose/SKILL.md`: `ceiling: stage:executing`; `discover-next/SKILL.md` step 4: `stage:decompose` when the picked stage is `clarify`, `stage:executing` when the picked stage is `decompose`. Without a ceiling on the decompose-stage case, a `pass-through`/`noop` outcome lands the item on `stage:executing` with `status:doing` in the same driving-loop turn, and the loop's own "no ceiling = keep going" rule would immediately invoke `fgos-executing` — silently starting a real code build from a command (`/fgOS:decompose <id>`) a person expects to run only the split-work judgment. Every one of these 3 skills must do exactly one stage's work per invocation, never cascade into the next. |
