...
| D2 | `/fgOS:discover-next` keeps its step 2 pick logic unchanged (`pickNextDiscoverItem`, `src/state/discover-pool.mjs` — ordering is correct, not touched). Only step 4 changes: instead of calling the bare verb on the picked id, hand that id to `fgos-coding-driving` with a **dynamic ceiling** — `stage:decompose` when the picked item's stage is `clarify` (stop right after clearing clarify; never let the loop drift into decompose judgment in the same turn), no ceiling when the picked stage is `decompose` (every decompose-verb outcome already exits the stage on its own: pass-through→executing, need-human→awaiting-human, decompose→anchored by children). |
