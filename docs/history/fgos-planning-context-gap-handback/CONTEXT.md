# fgos-planning: hand-back path for a material CONTEXT.md gap

## Feature boundary

`fgos-planning/SKILL.md` currently has exactly one documented exit:
success, handed to `fgos-validating` (line 176-177). It has no documented
...
product assumption itself (fgos-exploring's exclusive territory) or stall.

This item's scope is limited to that one gap: adding a documented
hand-back path from `fgos-planning` to `fgos-exploring` for a *material*
...
- reopening or overriding any decision already locked in an existing
  `CONTEXT.md` (forbidden, unchanged, per fgos-planning's own existing
  hard rule),
...
| ID | Decision |
|----|----------|
| D1 | The hand-back never moves `item.stage`. Verified against `src/state/fsm.mjs` (`TRANSITIONS`, no backward edges anywhere) and `src/state/workflow-stage-graphs.mjs` (`DOMAINS.coding.transitions`: `clarify->executing`, `clarify->decompose`, `decompose->executing`, `executing->compound-learn` — zero backward edges, no `decompose->clarify` edge exists). Correct mechanism: `fgos-planning` invokes `fgos-exploring`'s flow directly in the same session (Socratic lock, 3-test material/grounded/answerable filter, append a new D-ID decision to `CONTEXT.md`) while `item.stage` stays `decompose` the whole time — the same no-stage-move shape `fgos-validating` already uses when it hands an item back to `fgos-planning` directly (both stay in `decompose`, per `fgos-planning/SKILL.md`'s own framing that shaping and proving are a judgment split inside one stage, never two stage values). This corrects the original bug description's claim that the fix "mirrors fgos-validating's own existing hand back" one-for-one — that hand-back works only because validating and planning share the *same* stage (`decompose`) by design; `fgos-exploring` (`clarify`) and `fgos-planning` (`decompose`) are *different* stages under the default `coding` domain mapping (`workflow-stage-graphs.mjs` skillMap), so the real fix is a direct same-session skill invocation with no stage transition at all, not literal reuse of validating/planning's shared-stage mechanism. |
| D2 | The fix touches two files, not one: `fgos-planning/SKILL.md` (add the hand-back route for a material gap) and `fgos-exploring/SKILL.md` (correct its opening line — "This skill runs while a claimed item's `stage` is `clarify`" — which becomes inaccurate once `fgos-planning` invokes it directly while `item.stage` stays `decompose`). Grepped `fgos-exploring/SKILL.md` and `src/intake/*.mjs` for a mechanical `stage === 'clarify'` check: none exists — the opening line is descriptive framing only, not an enforced invariant, so `fgos-exploring`'s actual flow (Socratic lock steps, gate check) already works unchanged when invoked mid-`decompose`; only its own doc's opening description needs a one-line correction so it doesn't contradict this new usage. |
...
`fgos-validating/SKILL.md:75` lists **Assumptions** — "is every assumption
...
`fgos-planning`)" as an existing pattern. No new decision required here;
`fgos-planning/SKILL.md` only needs to name this path explicitly for the
material-gap case.

## Scout evidence

- `fgos-planning/SKILL.md:176-177` — confirmed by direct read: the only
  documented Handoff exit is `fgos-validating`, no return edge to
  `fgos-exploring`.
- `fgos-planning/SKILL.md:62-66` (Bootstrap) — confirmed: "the locked
...
- `fgos-exploring/SKILL.md` header — confirmed by direct read: "This skill
...
- `src/state/workflow-stage-graphs.mjs:64-69` `DOMAINS.coding.transitions`
  ...
  clarify` edge.
- `fgos-validating/SKILL.md:75,94-100` — Assumptions/feasibility-matrix
...
- **Material gap** — per `fgos-exploring/SKILL.md`'s own three-test filter
...
- Exact wording/placement of the new hand-back rule within
  `fgos-planning/SKILL.md`'s existing step numbering (e.g., as a new
  numbered flow step vs. a Hard Rules addition) is an editorial/
  implementation choice, not a product decision — left to `fgos-planning`.
- Whether the hand-back should also append anything to
  `fgos-planning/SKILL.md`'s own Red Flags list (mirroring
  `fgos-validating`'s red flags) is likewise an editorial call for
