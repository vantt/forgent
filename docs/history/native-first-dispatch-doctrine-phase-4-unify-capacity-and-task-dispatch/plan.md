...
|---|---|---|
| Shared decision helper (new) | High — no precedent in this repo, first native-Task branch ever built | `impact({target: "resolveExecutorConfig", direction: "upstream"})` before touching `dispatch.mjs`; new unit tests covering both branches (native-eligible vs not) |
| `judgeDiscovery`/`judgeDecompose` migration | Medium — touches `src/state/workflow-stage-graphs.mjs`-adjacent, Iron-Law-gated module (`src/intake/`) | `test/intake/discovery.test.mjs`, `test/intake/decompose.test.mjs` green; `classifyIronLaw` evidence per `docs/how-to/produce-failing-test-first-proof-for-an-iron-law-gated-diff.md` since this touches `src/intake/` (an Iron-Law `MODULE_RULES` entry) |
