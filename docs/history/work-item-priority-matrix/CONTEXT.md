...
- `tsk-17w` (filed this session) — `fgos-exploring` missing the `impact-analysis` capability-gate that `fgos-planning`/`fgos-validating`/`fgos-executing` already have (`tsk-1e4`). D3/D4/D8's rough-pass-vs-refined-pass split assumes this gap gets closed — `fgos-planning` should decide whether `tsk-17w` is a hard dependency of this item's rollout or can ship independently.
...
- Whether `tsk-17w` becomes a hard `deps` entry on the field-reorg half, or ships independently first.
