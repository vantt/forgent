# Plan: fgos-validating Gate — third case for already-created children
...
Edit `.claude/skills/fgos-validating/SKILL.md`'s Gate section (the block
...
- **Trigger:** `plan.md`'s step 4 already created the listed child pieces
  as real work items (via `fgos add --parent --footprint`, `fgos-planning`
...
Exact wording is this session's own to write during `fgos-code-implement`;
...
- `.claude/skills/fgos-validating/SKILL.md` (only)
...
carried to `fgos-validating` beyond the verify command itself.
...
```bash
S=$(awk "/^## Gate/,/^## Handoff/" .claude/skills/fgos-validating/SKILL.md); echo "$S" | grep -q "already created as real work items" && echo "$S" | grep -q "never.*decompose --children"
```
