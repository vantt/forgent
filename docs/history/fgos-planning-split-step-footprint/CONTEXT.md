...
item: tsk-3uz
stage: clarify
docsRef: docs/history/fgos-planning-split-step-footprint/
---

# CONTEXT — tsk-3uz: fgos-planning's split step không set --footprint cho con

## Feature boundary

`fgos-planning/SKILL.md`'s bước 5 ("Decide the split") tạo item con qua
...
"footprint" không xuất hiện ở đâu trong `fgos-planning/SKILL.md` (STR92
audit 2026-07-23, ý (2); tái xác nhận 2026-08-05 lúc `tsk-66o`'s
fgos-planning). Sửa văn bản bước 5 để LUÔN điền `--footprint` từ danh
...
|---|---|---|
| D1 | Fix thuần skill-prose — sửa `fgos-planning/SKILL.md`'s bước 5, không đụng code | Cơ chế `--footprint` đã hoạt động thật; gap chỉ là SKILL.md chưa hướng dẫn dùng nó |
...
- `grep -n "footprint" .claude/skills/fgos-planning/SKILL.md` → 0 kết quả (fresh, 2026-08-05).
