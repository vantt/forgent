...
- **`merge next`'s new branch** (`bin/fgos.mjs`, inside the existing `case 'next':`): when `ready.length === 0` and `blockedOnSync.length > 0`, resolve the top-ranked blockedOnSync id's root via `resolveRoot`, then call `sync-root` the same way this case already calls `approve` — recursively through `runVerb('sync-root', flags, [rootId], dir)` (D6's own "no parallel merge mechanism" contract, extended rather than broken). Branch on the outcome:
  - `outcome: 'synced'` — re-read `listWork`/`driftStatus`/`mergeReadiness` fresh (git state changed), and if something is now `ready`, fall straight into the existing `approve` path for that id, reporting `syncRoot: {id: rootId, outcome: 'synced'}` alongside. If still nothing is ready (e.g. a footprint conflict newly surfaced, or a different root still drifted), report `{picked: null, reason: 'nothing ready to merge', syncRoot: {...}}`.
  - `outcome: 'blocked'` (`merge-conflict` / `fgos-write-rejected` / `verify-fail`) — report `{picked: null, reason: 'blockedOnSync root sync failed', syncRoot: syncResult}` directly. No retry, no fallback to a different blockedOnSync candidate (D2).
  - Iron Law throw (`sync-root`'s own gate, same as `approve`'s) — caught the same way the existing Iron Law catch already handles `approve`, reported as `{picked: null, reason: 'blockedOnSync root trips Iron Law', syncRoot: {id: rootId, blocked: 'iron-law', message: err.message}}`. Never auto-acknowledges — same standing rule `approve`'s own Iron Law path already holds.
- **Skill docs**: `/fgOS:merge-next` SKILL.md gains one more outcome shape to relay (`syncRoot` alongside `picked`). `/fgOS:merge-loop` SKILL.md's stop-rule table gains one more real stop: any `syncRoot`-blocked outcome (conflict/iron-law/fgos-write-rejected/verify-fail) is a stop, same class as today's Iron Law trip — never silently retried on the same root next iteration. A `synced` outcome that still lands on "nothing ready" is not itself a new stop; the loop's existing frontier-empty stop already covers it once nothing changes across an iteration.

`impact-analysis` posture: **full** (`gitnexus`, `present` — confirmed this session). `fgos-executing` must run real `impact()` on `mergeReadiness`, the `merge` case in `bin/fgos.mjs`, and `sync-root` before editing any of them, per the repo's own gate — this plan's proof points below assume that check runs, not substitute for it.
...
| `/fgOS:merge-loop` stop-rule coverage for the new outcome | medium — an unhandled outcome shape could let the loop spin retrying the same blocked root forever | trace the skill's own stop-rule table by hand against every new outcome shape (prose review — `merge-loop` is a skill wrapper, not code under test) |
...
6. `plugins/fgOS/skills/merge-loop/SKILL.md` — stop-rule table gains the new blocked-sync stop
...
- `blockedOnSync` ordering via `orderByRank` is a pure implementation detail consistent with the file's existing pattern (`ready`/`mergeSets`/`supersededOut` all already use it) — not re-litigated with `fgos-exploring`, since CONTEXT.md's D2 already specifies the ordering rule this realizes; it was silent only on the *mechanism*, not the *behavior*.
