...
`fgos discover`/`fgos decompose` route through `judgeDiscovery`,
`judgeVerifySemanticCorrectness`, and `judgeDecompose`
(`src/intake/discovery.mjs`, `src/intake/decompose.mjs`), all of which share
...
timeout) can kill the whole `fgos discover`/`fgos decompose` process
...
- `src/intake/discovery.mjs`, `src/intake/decompose.mjs` — the three judge call sites.
