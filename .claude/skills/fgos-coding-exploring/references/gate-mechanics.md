...
combining a `git`-rooted command with a following `fgos` invocation.
Resolve `root` first, read its printed value, then substitute that
literal path into the second call.
