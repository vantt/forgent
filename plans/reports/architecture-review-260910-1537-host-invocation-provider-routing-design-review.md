...
Prerequisites trước P00 (Lead, ngoài cell): commit toàn bộ doc + track; thêm `cargo test/clippy/fmt/build` (cả dạng `rtk`) vào allowlist `claude-reviewer-herdr`; xác nhận bảng Decisions (default: linux-x64-glibc; preview; window 2 release; budget 25 ms/10 ms). **Đã xong 17:05, commit `4db6ee7e` + `43ba2b63`.**

**Mở rộng 17:25 (người dùng quyết):** track gánh luôn `fgctl` walking skeleton + `install.sh` cài thẳng từ GitHub release (không clone) + CI release. Thêm 5 cell: P11 fgctl crate/manifest verify/release store → P12 `fgctl init` + activation + shim `sh` + `fgos version --runtime-json` → P13 upgrade/repair/rollback/quarantine (full-suite) → P14 `install.sh` + test HTTP fixture + README → P15 `release.yml` (`gh release upload`, tag `v*`) + `scripts/ci-external-consumer.sh` chạy được cả local lẫn CI (full-suite); closeout thành P16. Bảng Decisions thêm 10 hàng chốt default cho 7 quyết định mở của `runtime-identity` §15 (release store `${XDG_STATE_HOME:-~/.local/state}/fgos/`, dir digest-only, topology V1 = main checkout, shim `sh`, acquire `--from dir|tar.gz|github`, preflight list, lease = `.fgos/main-checkout.lock` có sẵn, không GC, asset `fgos-*/fgctl-*/SHA256SUMS`). Với scope này track giao **trọn R1 cho một target**: `curl | sh` → `fgctl init` → `fgos` trong project là Rust; flip theo từng project qua `fgctl init`, npm `bin.fgos` giữ làm kênh tương thích.
