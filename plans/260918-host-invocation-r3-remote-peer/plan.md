...
[verification/r3-remote-peer-proof.md](../../docs/platform/host-invocation-routing/verification/r3-remote-peer-proof.md).
...
| R3-P1 focused tests | Passing | `cargo test --manifest-path herdr-plugin/Cargo.toml remote_invocation --quiet` passed 10 tests on 2026-09-18. | P1 has enough proof to be adopted as the base for P2. |
...
inventoried in
`docs/platform/host-invocation-routing/verification/r3-remote-peer-proof.md`;
closeout docs updated across the platform area. Only `GET /v1/runtime` is
migrated; all other routes remain on legacy `VerbGateway`.
...
This was a documentation freeze at R3-P0. R3-P1/P2 later added the remote projector/presenter and wired `GET /v1/runtime` to this frozen shape.
...
Current evidence: the focused scan passed 10 tests on 2026-09-18.
...
Historical prompt used to hand R3 to code-panel:

```text
Run the full Host Invocation R3 track from plans/260918-host-invocation-r3-remote-peer/plan.md.
...
R3 is closed as implemented preview for `GET /v1/runtime`. Future work should
not reopen R3 unless code evidence invalidates the selected route or proof.
Use this record as a closeout reference; new gateway migrations should open a
new route/component migration plan rather than extending R3.
