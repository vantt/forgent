...
| P14 | `da99a000` | Clean after 3 Lead fix rounds (the fix-round cap) across 4 sessions, dispatched and closed in parallel with P13 (disjoint leases); the doer's own assignment hit the 35-minute dispatch ceiling with real uncommitted work (install.sh + its test) and no README update, and its own test was independently found deadlocked by both the Lead and the session's own reviewer (spawnSync froze the event loop the in-process fixture http server needed to answer install.sh's own curl calls back to it) — Lead took over directly, fixed the deadlock (async spawn) and finished R8's README rewrite; round 1 reviewer clean, red-team's first pass (never dispatched in session 1) found 2 MEDIUM (unbounded curl/wget hang; a symlinked fgctl archive entry installs its external target's bytes), both fixed; round 2 reviewer found 1 more MEDIUM (the wget fallback path's idle-only timeout plus default 20 retries stayed effectively unbounded even with --timeout set), fixed with --tries=1 plus an external `timeout`-command wall-clock backstop; round 3 (reviewer only, final) clean | a few LOW/INFO accepted as documented caveats — a static source-regex test can't catch a semantically-equivalent timeout gap; tar's own symlinked-dir/hardlink escape refusal is environment behavior, not install.sh's own; a byte-drip server without coreutils `timeout` on PATH could still defeat wget's own idle timeout (not applicable to the only shipped target) (see doc) — see `docs/architect/agent-coordination/verification/rust-host-r1-kernel/p14.md` |
| P13 | `bafce71d` | Clean, full-suite gate green, after 2 Lead fix rounds (one short of the cap) across 4 sessions, dispatched and closed partly in parallel with P14 (disjoint leases); doer landed a real, complete commit on the first try (`upgrade`/`repair`/`verify` extending `status`, reusing P12's preflight/publish/tail pipeline); round 1 reviewer found 0 HIGH + 2 MEDIUM (quarantine recovery dead end; upgrade-away-from-quarantine rollback pointing at a gone release) + 9 LOW, red-team independently found 1 HIGH (`verify_workspace` publishing a stale pre-lock activation snapshot, a TOCTOU letting a concurrent upgrade be silently overwritten) + 1 MEDIUM (tampered-release repair refusing but never quarantining) + 1 LOW, all fixed; round 2 recheck (fresh session, wall-time budget exhausted) reviewer found a genuine regression — the round-1 MEDIUM fix reintroduced the exact TOCTOU red-team's HIGH had just closed, in `repair_workspace`'s own new quarantine branch — plus a second missed call site for the rollback-chaining rule, and red-team independently found a distinct crash-window MEDIUM (a release moved to quarantine but `activation.json` still reading `status: ready`, unrecoverable through any door); round 3 (fresh session, final) both reviewer and red-team confirmed the round-2 fix clean, red-team reproducing the crash window with a real gdb-driven SIGKILL and confirming `fgctl init --from` self-heals it | LOW (missing dedicated regression test for the init-door chain-past rule, though its upgrade-door twin has one); 2 INFO (a previousArtifactDigest that itself later gets quarantined — refused cleanly by design; a sub-ms same-digest lock-guard comparison window, not reproduced even under SIGSTOP injection); several LOW/L2-L9 DRY/duplication notes carried from round 1 (see doc) — see `docs/architect/agent-coordination/verification/rust-host-r1-kernel/p13.md` |
| P15 | `5694a746` | Clean, full-suite gate green, after 1 Lead fix round (two short of the cap) across 2 sessions; doer landed a real, complete commit on the first try (`scripts/ci-external-consumer.sh`, `.github/workflows/release.yml`, `ci.yml`'s new `external-consumer` job) covering R1-R4, before its own assignment hit the dispatch ceiling and was marked failed (real committed work, no loss, same pattern as earlier cells); round 1 reviewer found 0 HIGH + 1 MEDIUM (R1's digest assertion passed only via an undocumented manifest fallback — the real `SHA256SUMS` comparison was dead code), red-team independently found the same root cause as 1 HIGH (a run with the fgos `SHA256SUMS` entry replaced by 64 zeroes still exited 0) + 1 MEDIUM (the local asset server followed symlinks), both fixed by splitting the digest check into two non-substitutable sequential gates (archive-bytes-vs-`SHA256SUMS` first, tree-content-vs-verified-manifest second — the two hash spaces are architecturally distinct and the doer's fallback had conflated them) plus switching the server to `lstatSync`; round 2 (fresh session, final) both reviewer and red-team confirmed the fix clean, red-team rebuilding a fresh distribution and replaying checksum/symlink/traversal attacks with none reproduced | 4 LOW carried (Gate 1's checksum recompute runs after `fgctl init` already staged the archive rather than before; the phase file's own R1 prose still describes the pre-fix impossible single-gate design; `gh release create`'s line continuations collapsed cosmetically; `build-rust-distribution.mjs`'s new distribution-output-dir exemption has no dedicated unit test); no `CHANGELOG.md [Unreleased]` entry yet (deferred to P16, the track's own docs/changelog closeout cell); INFO — R5's real prerelease tag cut will hit GitHub's `/releases/latest` never resolving to a prerelease, needing `FGCTL_VERSION` explicitly (noted for when R5 is cut, CI's local server can't surface it) — see `docs/architect/agent-coordination/verification/rust-host-r1-kernel/p15.md` |
| P16 | `817ac7f3` | Clean, full-suite gate green (track-closing cell), after 3 Lead fix rounds (the fix-round cap) across 5 sessions (4 lost to pure infra — a dispatch-briefing race, a stale string-identity per-cwd dispatch lock that can only clear via TTL not pid-liveness reclaim, two host-OOM kills of the coordination wrapper under real shared-machine memory pressure, and a codex agent-startup timeout — none related to content); doer landed a real, complete commit on the first try (`CHANGELOG.md`, `docs/specs/reading-map.md`, `track-closeout.md`) before its own assignment was likewise falsely marked failed by the same dispatch-briefing race, real work preserved; round 1 reviewer found 0 HIGH + 1 MEDIUM (closeout's P05 deferred-findings entry omitted M1 and misattributed its disposition to M2) + 5 LOW (P11's INFO not carried forward; uncommitted "re-measurement" perf numbers cited; `cargo fmt` row overclaiming `herdr-plugin` coverage it doesn't have; wrong line citation), the Lead also independently caught and fixed a 6th issue (closeout's FULL_TEST table cited a narrower 88-test glob instead of the real whole-repo 6038-test one) before review even ran; round 2 reviewer found a more substantive MEDIUM in the round-1 fix itself (the `herdr-plugin` fmt-gap description undersold its real scope, 276 hunks across 12 files, and wrongly called all of it "unrelated to any cell this track shipped") + 1 LOW (self-contradictory perf sentence), fixed after the Lead verified precisely which 6 of 276 hunks were genuinely new (introduced by this track's own Phase 10 commits, since `herdr-plugin` is workspace-excluded from every `cargo fmt --all` this track ever ran) vs. the 270 reproducing identically on unmodified `main`; round 3 (final, first real red-team dispatch across 5 attempts) both reviewer and red-team confirmed clean, red-team independently reproducing the 270/6 hunk split and finding nothing across provenance/trace/reproduction/registry/documentation attacks | 1 LOW (informational naming nit — a deferred-finding sentence names a non-existent module path, file names in the same sentence still locate it correctly); 1 LOW (`herdr-plugin`'s 6 new formatting hunks from this track's own Phase 10 work, outside this docs-only cell's lease to fix, flagged for whoever next touches `herdr-plugin`) — see `docs/architect/agent-coordination/verification/rust-host-r1-kernel/p16.md` |

**Track `rust-host-r1-kernel` complete: P00-P16 all merged.**
