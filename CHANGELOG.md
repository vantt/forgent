...
- The live project dispatch configuration now binds every curated advisory and coding capability to a concrete executor: read-only `claude-bwrap` for `advise`, read-only `codex-readonly` for `code:review`/`code:debug`, `agy-herdr` for `code:implement`, and direct `claude` `cli-spawn` for `execute`/`code:test`/`code:refactor`. The direct path avoids the retired Herdr live-renderer entries; provider and model continue to derive from the selected executor plus the caller's tier, while protocol and skill definitions remain portable and unpinned.
