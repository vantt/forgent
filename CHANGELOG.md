...
- Disconnected-worktree guard for `submit`/`take`/`pick`/`move`/`edit`/`ask`/`answer`/`gate-approve`/`reject` (docs/history/agent-coordination-state-root, `coord_state_root_rfc_20260909`): these verbs now refuse — pointing at `--dir <mainRoot>` — when run with no `--dir` from a worktree made outside fgOS's own lifecycle (a plain `git worktree add`, or an external harness's own worktree tool) whose `.fgos/` is a real but disconnected git-tracked snapshot, never the shared main store. `src/runner/session.mjs` gains `isSessionWorktree(repoRoot)`, an unforgeable predicate (`.fgos` is a symlink AND its realpath equals the real main checkout's own `.fgos`) admitting the one legitimate non-main case, `fgos session start`. Read-only verbs, `evolve`/`coordination`/`session`/`goal`/`tool`/`merge`/`setup`/`doctor` (mixed-mode, each with a legitimate read-only invocation shape), and `approve`/`sync-root`/`promote-to-component`/`catchup`/`unclaim` (already independently guarded via their own `repoRoot`) are unaffected.
