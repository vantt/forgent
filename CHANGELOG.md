...
- The live project dispatch configuration now binds every curated advisory and coding capability to a concrete executor: read-only `claude-bwrap` for `advise`, read-only `codex-readonly` for `code:review`/`code:debug`, `agy-herdr` for `code:implement`, and `claude-herdr` for `execute`/`code:test`/`code:refactor`. Provider and model continue to derive from the selected executor plus the caller's tier; protocol and skill definitions remain portable and unpinned.
