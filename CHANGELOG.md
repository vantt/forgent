...
- `fgos stale` now also reports runs nothing is left to finish — a `run.json` still saying `running` with no dispatch process behind it — alongside the stuck-item and post-delivery advisories it already gave. It stays read-only by default, which is what the verb documents about itself, so an orphaned run is described (`wouldBecome`, `hasWorkerResult`) and not touched; `fgos stale --reconcile` is the explicit ask that writes each real outcome back. Liveness is never probed here — `stale` holds no herdr client and starts none — so a run reads as `settled` when the worker's own result file is on disk and `unknown` otherwise, and specifically never as `died`, which would be an assertion about a process nobody looked at.
